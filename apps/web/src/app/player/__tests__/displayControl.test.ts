...
// ─────────────────────────────────────────────────────────────────
// RESET_CANVAS (2026-09-01, G55 field find) — the device-side LED canvas
// pin. A 2160×3840 Goodview drew its template in a ~720×1280 box in one
// corner: the on-screen "Resize for LED" editor had left `edu_canvasW/H`
// in localStorage, the pin script prefers that over the panel's real size,
// and the server had no record of it — so no dashboard control could reach
// it. This lane clears the pin and asks for a reload; like OPEN_SETUP it
// never touches `displayApply`, and it never runs over a displayed alert.
// ─────────────────────────────────────────────────────────────────
describe('RESET_CANVAS', () => {
  const { resetDeviceCanvasPin, DEVICE_CANVAS_KEYS } = require('../displayControl');

  beforeEach(() => {
    window.localStorage.clear();
  });

  it('clears every device-side canvas key, asks the APK to reload, never calls displayApply', () => {
    window.localStorage.setItem('edu_canvasW', '720');
    window.localStorage.setItem('edu_canvasH', '1280');
    window.localStorage.setItem('edu_fitMode', 'contain');
    fireMock.mockImplementation((m: string) => m === 'reload');

    const res = dispatchDisplayControl(
      envelope({ action: 'RESET_CANVAS' }),
      'screen-1',
      ctx(),
      'WS',
      sink(),
    ...
    expect(res).toEqual({ status: 'canvas-reset', action: 'RESET_CANVAS' });
    for (const k of DEVICE_CANVAS_KEYS) expect(window.localStorage.getItem(k)).toBeNull();
    expect(fireMock).toHaveBeenCalledWith('reload');
    expect(callMock).not.toHaveBeenCalled();
  });

  it('reports whether a pin was actually present', () => {
    fireMock.mockImplementation((m: string) => m === 'reload');
    expect(resetDeviceCanvasPin('c1', 'WS')).toBe(false);
    window.localStorage.setItem('edu_canvasW', '720');
    expect(resetDeviceCanvasPin('c2', 'WS')).toBe(true);
    expect(window.localStorage.getItem('edu_canvasW')).toBeNull();
  });

  it('is dropped while an alert is displayed, leaving the pin untouched', () => {
    window.localStorage.setItem('edu_canvasW', '720');
    fireMock.mockImplementation((m: string) => m === 'reload');
    const res = dispatchDisplayControl(
      envelope({ action: 'RESET_CANVAS' }),
    ...
    expect(res).toEqual({ status: 'dropped', reason: 'emergency' });
    expect(window.localStorage.getItem('edu_canvasW')).toBe('720');
    expect(fireMock).not.toHaveBeenCalledWith('reload');
  });
});
