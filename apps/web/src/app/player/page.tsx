...
function PlayerPage() {
  ...
  useEffect(() => {
    ...
    // v1.0.39 — Native URL overlay. When the active playlist item is
    // a URL (mime=text/html), call EduCmsNative.showUrlOverlay(url).
    // The native side flips a SECOND WebView visible on top of the
    // React player and loads the URL there. The React player keeps
    // running underneath — playlist scheduler, heartbeats, emergency
    // listeners all stay alive. When the playlist's duration timer
    // fires below and we advance to the next item, the cleanup
    // hideUrlOverlay() effect (separate useEffect) tears the overlay
    // down.
    //
    // Why this design (vs the broken v1.0.36-37 fullscreen-nav approach):
    //   - No "navigate React WebView away → back" round-trip
    //   - No connecting splash flash between URL items
    //   - No reload loop on single-URL or URL-only playlists
    //   - React stays alive → emergency overrides still take effect
    //     instantly even while a URL is showing
    //   - Same WebView lifecycle as Goodview/Yodeck/OptiSigns use
    //
    // The duration timer below still fires normally; setCurrentIndex
    // advances; the next useEffect tick either calls showUrlOverlay
    // with a different URL, or calls hideUrlOverlay (next item is
    // not a URL).
  ...
  // v1.0.39 — Drive the URL overlay WebView based on the active item.
  // Runs whenever phase, playlist, or currentIndex changes. Three
  // outcomes:
  //   - phase !== 'playing' → hide overlay (covers emergency, offline,
  //     pairing, connecting — emergency content MUST be visible
  //     immediately, not hidden behind a third-party URL)
  //   - active item is URL → showUrlOverlay(url)
  //   - active item is non-URL → hideUrlOverlay()
  //
  // Idempotent on the native side — calling showUrlOverlay with the
  // same URL while already visible is a no-op (doesn't re-init the
  // third-party page). hideUrlOverlay on an already-hidden overlay
  // is also a no-op.
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const bridge = (window as any).EduCmsNative;
    const showFn = bridge && typeof bridge.showUrlOverlay === 'function' ? bridge.showUrlOverlay.bind(bridge) : null;
    const hideFn = bridge && typeof bridge.hideUrlOverlay === 'function' ? bridge.hideUrlOverlay.bind(bridge) : null;
    if (!showFn || !hideFn) return; // no native bridge — iframe path renders inline

    const tearDown = () => {
      try { hideFn(); } catch (e) { console.warn('[Player] hideUrlOverlay failed', e); }
    };

    if (phase !== 'playing') { tearDown(); return tearDown; }
    if (!playlist || playlist.length === 0) { tearDown(); return tearDown; }
    const sorted = [...playlist].sort((a: any, b: any) => a.sequenceOrder - b.sequenceOrder);
    if (sorted.length === 0) { tearDown(); return tearDown; }
    const idx = ((currentIndex % sorted.length) + sorted.length) % sorted.length;
    const activeItem = sorted[idx];
    const mime = activeItem?.asset?.mimeType || '';
    const fileUrl = activeItem?.asset?.fileUrl || '';
    const isUrlItem = mime === 'text/html' && /^https?:\/\//i.test(fileUrl);
    try {
      if (isUrlItem) {
        console.log(`[Player] showUrlOverlay → ${fileUrl}`);
        showFn(fileUrl);
      } else {
        hideFn();
      }
    } catch (e) {
      console.warn('[Player] URL overlay bridge call failed', e);
    }
    // Cleanup: on phase change to non-playing OR component unmount,
    // hide the overlay. Between URL-to-URL transitions the next effect
    // run will showUrlOverlay with the new URL (idempotent — no flicker).
    return () => {
      // No-op cleanup between same-effect runs; phase change cleanup
      // handled by the early-return tearDown paths above.
    };
  }, [phase, playlist, currentIndex]);
        ...
        <div className={`relative w-full h-full flex items-center justify-center ${isPlaylistInteractive ? '' : 'pointer-events-none'}`}>
          {sorted.map((item, index) => {
            ...
            if (isWeb) {
              ...
              // regression.
              // v1.0.39 — when the native overlay bridge is available
              // AND this is a URL asset (not a PDF), the URL renders
              // in the native overlay WebView (driven by the
              // showUrlOverlay/hideUrlOverlay effect above). Render a
              // transparent placeholder so the inactive transition
              // physics still work, but no actual iframe — that would
              // double-render the URL underneath the overlay.
              if (!isPdf && typeof window !== 'undefined') {
                const bridge = (window as any).EduCmsNative;
                if (bridge && typeof bridge.showUrlOverlay === 'function') {
                  return <div key={item.id} className={classes + ' bg-black'} aria-label="url-overlay-placeholder" />;
                }
              }
