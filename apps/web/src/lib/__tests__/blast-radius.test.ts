...
describe('computeBlastRadius — double-sided displays', () => {
  // One physical display, two Screen rows. The back mirrors the front, so it
  // has no schedules of its own and none can ever be written for it — it
  // simply resolves the front's. The reach has to say so, or the operator is
  // told "1 screen" while two panes of glass change.
  const DOUBLE_SIDED = [
    { id: 'd1', name: 'Entrance Display' },
    { id: 'd1b', name: 'Entrance Display — Back', faceOfScreenId: 'd1', faceContentMode: 'MIRROR' },
  ];

  it('counts a mirroring back side when the front is picked', () => {
    const r = computeBlastRadius({
      screens: DOUBLE_SIDED,
      groups: [],
      selectedScreenIds: ['d1'],
    });
    expect(r.screenCount).toBe(2);
    expect(r.screenNames).toEqual(
      expect.arrayContaining(['Entrance Display', 'Entrance Display — Back']),
    );
    expect(blastRadiusLine(r)).toBe('Publishes to 2 screens');
  });

  it('does NOT count a side that has its own content', () => {
    // 'OWN' is an ordinary screen: it plays what IT is scheduled, so a
    // publish to the front must not claim to reach it.
    const r = computeBlastRadius({
      screens: [
        DOUBLE_SIDED[0],
        { ...DOUBLE_SIDED[1], faceContentMode: 'OWN' },
      ],
      groups: [],
      selectedScreenIds: ['d1'],
    });
    expect(r.screenCount).toBe(1);
    expect(r.screenNames).toEqual(['Entrance Display']);
  });

  it('an OWN side is reached when it is picked directly', () => {
    const r = computeBlastRadius({
      screens: [DOUBLE_SIDED[0], { ...DOUBLE_SIDED[1], faceContentMode: 'OWN' }],
      groups: [],
      selectedScreenIds: ['d1b'],
    });
    expect(r.screenCount).toBe(1);
    expect(r.screenNames).toEqual(['Entrance Display — Back']);
  });

  it('a mirroring side follows its front through a picked GROUP too', () => {
    // Group publishes are the common case, and the back is in the group as
    // well — but it would be counted twice if the mirror rule double-added.
    const r = computeBlastRadius({
      screens: DOUBLE_SIDED,
      groups: [{ id: 'gd', name: 'Entrance', screens: [{ id: 'd1', name: 'Entrance Display' }] }],
      selectedScreenIds: ['d1'],
      selectedGroupIds: ['gd'],
    });
    expect(r.screenCount).toBe(2);
  ...
  it('a missing faceContentMode reads as MIRROR — the safe default', () => {
    const r = computeBlastRadius({
      screens: [DOUBLE_SIDED[0], { id: 'd1b', name: 'Back', faceOfScreenId: 'd1' }],
      groups: [],
      selectedScreenIds: ['d1'],
    });
    expect(r.screenCount).toBe(2);
  ...
  it('leaves an ordinary fleet completely untouched', () => {
    // The regression guard: no face fields anywhere → byte-identical answer.
    const r = computeBlastRadius({
      screens: SCREENS,
      groups: GROUPS,
      selectedScreenIds: ['s1', 's4'],
    });
    expect(r.screenCount).toBe(2);
    expect(r.screenNames).toEqual(['Lobby North', 'Gym Ribbon']);
  });
});
