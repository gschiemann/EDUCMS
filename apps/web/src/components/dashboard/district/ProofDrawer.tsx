    ...
    <li className="flex-1 min-w-0 flex flex-col items-center" data-testid={`step-${label.toLowerCase()}`} data-state={state}>
      <div className="w-full flex items-center">
        ...
        <span
          ...
          aria-hidden
        >
          {state === 'complete' && <Check className="w-3.5 h-3.5" />}
        </span>
        {last ? spacer : solid}
      </div>
      <span className="mt-1.5 text-[10.5px] font-black text-slate-700 text-center leading-tight">
        {label}
      </span>
      <span className="mt-0.5 text-[10px] font-semibold text-slate-400 text-center leading-tight">{value}</span>
    </li>
  );
}
...
export function ProofDrawer({
  ...
  return (
    <div className="fixed top-0 right-0 bottom-0 left-0 z-[9999]" role="dialog" aria-modal="true" aria-label={`Screens for ${deployment.label}`}>
      ...
      >
        {/* ─── Header ────────────────────────────────────────────── */}
        {/* Panel language from the Content Control mock: a brand eyebrow, a
            big title that can carry a long campaign name, then the numbers —
            in that order, so the operator reads WHAT before HOW FAR. */}
        <div className="px-6 pt-5 pb-4 border-b border-slate-200 shrink-0">
          <div className="flex items-start gap-3">
            <div className="min-w-0 flex-1">
              ...
              >
                Now pushing across the fleet
              </p>
              <h2 className="mt-1 text-[19px] leading-tight font-black text-slate-900 truncate" title={deployment.label}>
                {deployment.label}
              </h2>
              <p className="mt-1.5 text-[13px] font-bold text-slate-600">
                {deployment.convergence.converged} of {deployment.targetCount} confirmed
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {screens.length === 0 ? (
            ...
            <>
              <p className="text-[13px] font-black text-slate-800 mb-2.5">
                Still waiting
                <span className="ml-2 font-semibold text-slate-400">
                  {screens.length} screen{screens.length === 1 ? '' : 's'} across {groups.size} {noun}
                </span>
              </p>
              {Array.from(groups.entries()).map(([location, rows]) => (
                <div key={location} className="rounded-2xl border border-slate-200 mb-3 overflow-hidden">
                  <div className="px-4 py-2.5 border-b border-slate-100 bg-slate-50/60 text-[12px] font-black text-slate-700 truncate">
                    {location}
                  </div>
                  <ul>
                    {rows.map((s) => (
          ...
          {unnamed > 0 && (
            <div className="rounded-2xl border border-slate-200 bg-slate-50/60 px-4 py-3.5">
              <h3 className="text-[12.5px] font-black text-slate-800">
