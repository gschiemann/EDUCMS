 ...
 */
const pickStatic = <K extends keyof Required<MsFieldnotesConfig>>(
...
export function MsFieldnotesWidget({ config, live }: { config?: MsFieldnotesConfig; live?: boolean }) {
  const cfg = config || {};
  ...
  const { now, weather } = useLiveTemplateData({
    ...
    weatherOverride: cfg['weather.v'],
  });

  const pick = <K extends keyof Required<MsFieldnotesConfig>>(key: K): string => {
    const v = cfg[key];
    if (v !== undefined && v !== '') return v as string;
    if (live) {
      if (key === 'brand.day') return fmt.shortDateUpper(now);
      if (key === 'clock.v') return fmt.time12NoSuffix(now);
      if (key === 'weather.v' && weather) return `${weather.tempF}°`;
    }
    ...
    >
      ...
      <header className="ms-fn-mast">
        <div className="ms-fn-lockup" data-widget="school">
          ...
          <div className="ms-fn-info">
            <div className="ms-fn-ey">
              <span data-field="school.ey" style={{ whiteSpace: 'pre-wrap' }}>{pick('school.ey')}</span>
            </div>
            <div className="ms-fn-ttl">
              <span data-field="school.name" style={{ whiteSpace: 'pre-wrap' }}>{pick('school.name')}</span>{' '}
              <em data-field="school.amp" style={{ whiteSpace: 'pre-wrap' }}>{pick('school.amp')}</em>{' '}
              <span data-field="school.mascot" style={{ whiteSpace: 'pre-wrap' }}>{pick('school.mascot')}</span>
            </div>
        ...
        <div className="ms-fn-tiles">
          <div className="ms-fn-tile" data-widget="weather">
            <span className="ms-fn-k" data-field="weather.k" style={{ whiteSpace: 'pre-wrap' }}>{pick('weather.k')}</span>
            <span className="ms-fn-v ms-fn-cool" data-field="weather.v" style={{ whiteSpace: 'pre-wrap' }}>{pick('weather.v')}</span>
            <span className="ms-fn-cap" data-field="weather.cap" style={{ whiteSpace: 'pre-wrap' }}>{pick('weather.cap')}</span>
          </div>
          <div className="ms-fn-tile" data-widget="clock">
            <span className="ms-fn-k" data-field="clock.k" style={{ whiteSpace: 'pre-wrap' }}>{pick('clock.k')}</span>
            <span className="ms-fn-v" data-field="clock.v" style={{ whiteSpace: 'pre-wrap' }}>{pick('clock.v')}</span>
            <span className="ms-fn-cap" data-field="clock.cap" style={{ whiteSpace: 'pre-wrap' }}>{pick('clock.cap')}</span>
          </div>
          <div className="ms-fn-tile" data-widget="bell">
            <span className="ms-fn-k" data-field="bell.k" style={{ whiteSpace: 'pre-wrap' }}>{pick('bell.k')}</span>
            <span className="ms-fn-v ms-fn-warm" data-field="bell.v" style={{ whiteSpace: 'pre-wrap' }}>{pick('bell.v')}</span>
            <span className="ms-fn-cap" data-field="bell.cap" style={{ whiteSpace: 'pre-wrap' }}>{pick('bell.cap')}</span>
          </div>
        </div>
      </header>
      ...
      <section className="ms-fn-spread">
        ...
        <div className="ms-fn-page ms-fn-left">
          <span className="ms-fn-pageno" data-widget="pageno-l">
            <span data-field="pageno.l" style={{ whiteSpace: 'pre-wrap' }}>{pick('pageno.l')}</span>
          </span>
          ...
          <div className="ms-fn-inner">
            <div className="ms-fn-specimen-row">
              ...
              <div className="ms-fn-specimen" data-widget="specimen" aria-hidden="true">
                ...
                <div className="ms-fn-latin" data-field="specimen.latin" style={{ whiteSpace: 'pre-wrap' }}>{pick('specimen.latin')}</div>
                <div className="ms-fn-common" data-field="specimen.common" style={{ whiteSpace: 'pre-wrap' }}>{pick('specimen.common')}</div>
                <div className="ms-fn-meta" data-field="specimen.meta" style={{ whiteSpace: 'pre-wrap' }}>{pick('specimen.meta')}</div>
              </div>

              {/* Hero copy */}
              <div className="ms-fn-hero-copy" data-widget="greeting">
                <div className="ms-fn-chapter" data-field="greeting.chapter" style={{ whiteSpace: 'pre-wrap' }}>{pick('greeting.chapter')}</div>
                <h1 className="ms-fn-h1">
                  <span data-field="greeting.h1a" style={{ whiteSpace: 'pre-wrap' }}>{pick('greeting.h1a')}</span>{' '}
                  <em data-field="greeting.h1b" style={{ whiteSpace: 'pre-wrap' }}>{pick('greeting.h1b')}</em>
                  <br />
                  <u data-field="greeting.h1c" style={{ whiteSpace: 'pre-wrap' }}>{pick('greeting.h1c')}</u>{' '}
                  <span data-field="greeting.h1d" style={{ whiteSpace: 'pre-wrap' }}>{pick('greeting.h1d')}</span>
                </h1>
                ...
                <div className="ms-fn-signature">
                  <span className="ms-fn-scribble" data-field="greeting.scribble" style={{ whiteSpace: 'pre-wrap' }}>
                    {pick('greeting.scribble')}
                  </span>
                  <span className="ms-fn-stamp">
                    Approved by
                    <br />
                    <b data-field="greeting.stampname" style={{ whiteSpace: 'pre-wrap' }}>{pick('greeting.stampname')}</b>
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="ms-fn-page ms-fn-right">
          <span className="ms-fn-pageno" data-widget="pageno-r">
            <span data-field="pageno.r" style={{ whiteSpace: 'pre-wrap' }}>{pick('pageno.r')}</span>
          </span>
          <div className="ms-fn-inner-right">
            ...
            <div className="ms-fn-polaroid" data-widget="spotlight">
              ...
              <div className="ms-fn-caption">
                ...
                <span className="ms-fn-cap-name" data-field="spotlight.name" style={{ whiteSpace: 'pre-wrap' }}>{pick('spotlight.name')}</span>
                <span data-field="spotlight.note" style={{ whiteSpace: 'pre-wrap' }}>{pick('spotlight.note')}</span>
              </div>
            </div>

            {/* Countdown */}
            <div className="ms-fn-countdown" data-widget="countdown">
              <div className="ms-fn-cd-ey" data-field="countdown.ey" style={{ whiteSpace: 'pre-wrap' }}>{pick('countdown.ey')}</div>
              <div>
                <div className="ms-fn-cd-num">
                  <span data-field="countdown.num" style={{ whiteSpace: 'pre-wrap' }}>{pick('countdown.num')}</span>
                  <span data-field="countdown.unit" style={{ whiteSpace: 'pre-wrap' }}>{pick('countdown.unit')}</span>
                </div>
                <div className="ms-fn-cd-label" data-field="countdown.label" style={{ whiteSpace: 'pre-wrap' }}>{pick('countdown.label')}</div>
              </div>
              <div className="ms-fn-cd-cap" data-field="countdown.cap" style={{ whiteSpace: 'pre-wrap' }}>{pick('countdown.cap')}</div>
            </div>
            ...
            <div className="ms-fn-agenda" data-widget="agenda">
              <div className="ms-fn-agenda-head">
                <h2 className="ms-fn-agenda-h2">
                  <span data-field="agenda.h" style={{ whiteSpace: 'pre-wrap' }}>{pick('agenda.h')}</span>{' '}
                  <em data-field="agenda.h2" style={{ whiteSpace: 'pre-wrap' }}>{pick('agenda.h2')}</em>
                </h2>
      ...
      <section className="ms-fn-margin">
        ...
        <div className="ms-fn-lunch ms-fn-card-base" data-widget="lunch">
          <div className="ms-fn-card-ey" data-field="lunch.ey" style={{ whiteSpace: 'pre-wrap' }}>{pick('lunch.ey')}</div>
          <h3 className="ms-fn-card-h3" data-field="lunch.h" style={{ whiteSpace: 'pre-wrap' }}>{pick('lunch.h')}</h3>
          <div className="ms-fn-entree" data-field="lunch.entree" style={{ whiteSpace: 'pre-wrap' }}>{pick('lunch.entree')}</div>
          <div className="ms-fn-sides" data-field="lunch.sides" style={{ whiteSpace: 'pre-wrap' }}>{pick('lunch.sides')}</div>
          <div className="ms-fn-quote" data-field="lunch.quote" style={{ whiteSpace: 'pre-wrap' }}>{pick('lunch.quote')}</div>
        </div>

        {/* Clubs */}
        <div className="ms-fn-clubs ms-fn-card-base" data-widget="clubs">
          <div className="ms-fn-card-ey" data-field="clubs.ey" style={{ whiteSpace: 'pre-wrap' }}>{pick('clubs.ey')}</div>
          <h3 className="ms-fn-card-h3" data-field="clubs.h" style={{ whiteSpace: 'pre-wrap' }}>{pick('clubs.h')}</h3>
              ...
              <li key={i} className="ms-fn-clubs-li" data-widget={`clubs.${i}`}>
                ...
                <span>
                  <b data-field={`clubs.${i}.n`} style={{ whiteSpace: 'pre-wrap' }}>
                    {pick(`clubs.${i}.n` as keyof Required<MsFieldnotesConfig>)}
                  </b>
                  <span className="ms-fn-where" data-field={`clubs.${i}.r`} style={{ whiteSpace: 'pre-wrap' }}>
                    {pick(`clubs.${i}.r` as keyof Required<MsFieldnotesConfig>)}
                  </span>
                </span>
                <span className="ms-fn-when" data-field={`clubs.${i}.w`} style={{ whiteSpace: 'pre-wrap' }}>
                  {pick(`clubs.${i}.w` as keyof Required<MsFieldnotesConfig>)}
                </span>
              </li>
            ))}
          </ul>
        ...
        <div className="ms-fn-buses ms-fn-card-base" data-widget="buses">
          <div className="ms-fn-card-ey" data-field="buses.ey" style={{ whiteSpace: 'pre-wrap' }}>{pick('buses.ey')}</div>
          <h3 className="ms-fn-card-h3" data-field="buses.h" style={{ whiteSpace: 'pre-wrap' }}>{pick('buses.h')}</h3>
            ...
            {([0, 1, 2, 3] as const).map((i) => (
              <li key={i} className="ms-fn-buses-li" data-widget={`buses.${i}`}>
                <span className="ms-fn-bus-num" data-field={`buses.${i}.num`} style={{ whiteSpace: 'pre-wrap' }}>
                  {pick(`buses.${i}.num` as keyof Required<MsFieldnotesConfig>)}
                </span>
                <span className="ms-fn-bus-rt">
                  <b data-field={`buses.${i}.rt`} style={{ whiteSpace: 'pre-wrap' }}>
                    {pick(`buses.${i}.rt` as keyof Required<MsFieldnotesConfig>)}
                  </b>
                  <span data-field={`buses.${i}.note`} style={{ whiteSpace: 'pre-wrap' }}>
                    {pick(`buses.${i}.note` as keyof Required<MsFieldnotesConfig>)}
                  </span>
                </span>
                <span
                 ...
                 style={{ whiteSpace: 'pre-wrap' }}>
                  {pick(`buses.${i}.st` as keyof Required<MsFieldnotesConfig>)}
                </span>
              </li>
            ))}
          </ul>
        ...
        <div className="ms-fn-shouts ms-fn-card-base" data-widget="shouts">
          <div className="ms-fn-card-ey" data-field="shouts.ey" style={{ whiteSpace: 'pre-wrap' }}>{pick('shouts.ey')}</div>
          <h3 className="ms-fn-card-h3" data-field="shouts.h" style={{ whiteSpace: 'pre-wrap' }}>{pick('shouts.h')}</h3>
              ...
              return (
                <li key={i} className="ms-fn-shouts-li" data-widget={`shouts.${i}`}>
                  <span className={`ms-fn-ic ${klass}`.trim()} data-field={`shouts.${i}.ic`} style={{ whiteSpace: 'pre-wrap' }}>
                    {pick(`shouts.${i}.ic` as keyof Required<MsFieldnotesConfig>)}
                  </span>
                  <span className="ms-fn-name">
                    <span data-field={`shouts.${i}.name`} style={{ whiteSpace: 'pre-wrap' }}>
                      {pick(`shouts.${i}.name` as keyof Required<MsFieldnotesConfig>)}
                    </span>
                    <span data-field={`shouts.${i}.note`} style={{ whiteSpace: 'pre-wrap' }}>
                      {pick(`shouts.${i}.note` as keyof Required<MsFieldnotesConfig>)}
                    </span>
                  </span>
                </li>
              );
      ...
      <footer className="ms-fn-ticker" data-widget="ticker">
        <div className="ms-fn-ticker-tag">
          <span data-field="ticker.tag" style={{ whiteSpace: 'pre-wrap' }}>{pick('ticker.tag')}</span>
        </div>
