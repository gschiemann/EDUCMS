...
/** Top-level picker (config-only, no live data) — used by code paths that
 *  don't have access to the live hook (e.g. defaults preview). */
const pickStatic = <K extends keyof Required<MsPlaylistConfig>>(
...
export function MsPlaylistWidget({ config, live }: { config?: MsPlaylistConfig; live?: boolean }) {
  const cfg = config || {};

  // Live clock + weather. Operator-typed values still WIN — the hook is
  // the FALLBACK for fields the teacher hasn't customized. Teachers only
  // need to type things only they know (cycle day, school name, etc.);
  // everything else (current time, today's date, outside temp) is auto.
  ...
  const pick = <K extends keyof Required<MsPlaylistConfig>>(key: K): string => {
    const v = cfg[key];
    // Operator override always wins
    if (v !== undefined && v !== '') return v as string;
    // Live-data overrides for known auto fields (only when live)
    if (live) {
    ...
    >
      ...
      <header className="ms-pl-appbar">
        <div className="ms-pl-brand" data-widget="brand">
          <div className="ms-pl-mark" data-field="brand.logoUrl" aria-label="School logo">
            {pick('brand.logoUrl') ? (
              <img
          ...
          <div className="ms-pl-brand-info">
            <div className="ms-pl-eye" data-field="brand.eyebrow" style={{ whiteSpace: 'pre-wrap' }}>{pick('brand.eyebrow')}</div>
        ...
        <div className="ms-pl-device" data-widget="device">
          ...
          <div className="ms-pl-device-info">
            <div className="ms-pl-device-k" data-field="device.label" style={{ whiteSpace: 'pre-wrap' }}>{pick('device.label')}</div>
            <div className="ms-pl-device-v" data-field="device.name" style={{ whiteSpace: 'pre-wrap' }}>{pick('device.name')}</div>
          </div>
        </div>

        <div className="ms-pl-meta-tiles">
          <div className="ms-pl-mt" data-widget="clock">
            <span className="ms-pl-mt-k" data-field="clock.label" style={{ whiteSpace: 'pre-wrap' }}>{pick('clock.label')}</span>
          ...
          <div className="ms-pl-mt" data-widget="weather">
            <span className="ms-pl-mt-k" data-field="weather.label" style={{ whiteSpace: 'pre-wrap' }}>{pick('weather.label')}</span>
            <span className="ms-pl-mt-v ms-pl-cool" data-field="weather.temp" style={{ whiteSpace: 'pre-wrap' }}>{pick('weather.temp')}</span>
          </div>
          <div className="ms-pl-mt" data-widget="countdown">
            <span className="ms-pl-mt-k" data-field="countdown.label" style={{ whiteSpace: 'pre-wrap' }}>{pick('countdown.label')}</span>
            <span className="ms-pl-mt-v ms-pl-green" data-field="countdown.value" style={{ whiteSpace: 'pre-wrap' }}>{pick('countdown.value')}</span>
          </div>
      ...
      <section className="ms-pl-hero">
        ...
        <div className="ms-pl-cover" data-widget="cover">
          <div className="ms-pl-albumtxt">
            <div>
              <div className="ms-pl-cover-top" data-field="cover.top" style={{ whiteSpace: 'pre-wrap' }}>{pick('cover.top')}</div>
            </div>
            <div className="ms-pl-cover-big" data-field="cover.big" style={{ whiteSpace: 'pre-wrap' }}>{pick('cover.big')}</div>
        ...
        <div className="ms-pl-now" data-widget="now">
          <div className="ms-pl-now-eyebrow" data-field="now.eyebrow" style={{ whiteSpace: 'pre-wrap' }}>{pick('now.eyebrow')}</div>
          ...
          <div className="ms-pl-chips">
            <span className="ms-pl-chip ms-pl-chip-exp" data-field="now.chipE" style={{ whiteSpace: 'pre-wrap' }}>{pick('now.chipE')}</span>
            <span className="ms-pl-chip" data-field="now.chip1" style={{ whiteSpace: 'pre-wrap' }}>{pick('now.chip1')}</span>
            <span className="ms-pl-chip ms-pl-chip-green" data-field="now.chip2" style={{ whiteSpace: 'pre-wrap' }}>{pick('now.chip2')}</span>
            <span className="ms-pl-chip-meta" data-field="now.meta" style={{ whiteSpace: 'pre-wrap' }}>{pick('now.meta')}</span>
          </div>
          ...
          <div className="ms-pl-progress">
            ...
            <div className="ms-pl-times">
              <span className="ms-pl-time-left" data-field="now.elapsed" style={{ whiteSpace: 'pre-wrap' }}>{pick('now.elapsed')}</span>
      ...
      <section className="ms-pl-middle">
        ...
        <div className="ms-pl-queue" data-widget="queue">
          <div className="ms-pl-queue-head">
            ...
            <div className="ms-pl-queue-meta">
              ...
              <div className="ms-pl-queue-pill" data-field="queue.pill" style={{ whiteSpace: 'pre-wrap' }}>{pick('queue.pill')}</div>
            </div>
          </div>
              ...
              return (
                ...
                >
                  <span className="ms-pl-idx" data-field={`queue.${i}.idx`} style={{ whiteSpace: 'pre-wrap' }}>
                    {pick(`queue.${i}.idx` as keyof Required<MsPlaylistConfig>)}
                  </span>
        ...
        <aside className="ms-pl-side">
          ...
          <div className="ms-pl-card ms-pl-featured" data-widget="featured">
            <div className="ms-pl-card-ey" data-field="featured.eyebrow" style={{ whiteSpace: 'pre-wrap' }}>{pick('featured.eyebrow')}</div>
            <h3 className="ms-pl-card-h3" data-field="featured.title" style={{ whiteSpace: 'pre-wrap' }}>{pick('featured.title')}</h3>
            <div className="ms-pl-featured-row">
              <div className="ms-pl-featured-pic" data-field="featured.art" style={{ whiteSpace: 'pre-wrap' }}>{pick('featured.art')}</div>
              <div className="ms-pl-featured-info">
                <div className="ms-pl-featured-t" data-field="featured.name" style={{ whiteSpace: 'pre-wrap' }}>{pick('featured.name')}</div>
                <div className="ms-pl-featured-s" data-field="featured.desc" style={{ whiteSpace: 'pre-wrap' }}>{pick('featured.desc')}</div>
                <span className="ms-pl-featured-tag" data-field="featured.tag" style={{ whiteSpace: 'pre-wrap' }}>{pick('featured.tag')}</span>
              </div>
            </div>
          </div>
          ...
          <div className="ms-pl-card ms-pl-recent" data-widget="recent">
            <div className="ms-pl-recent-head">
              <div>
                <div className="ms-pl-card-ey" data-field="recent.eyebrow" style={{ whiteSpace: 'pre-wrap' }}>{pick('recent.eyebrow')}</div>
                <h3 className="ms-pl-card-h3" data-field="recent.title" style={{ whiteSpace: 'pre-wrap' }}>{pick('recent.title')}</h3>
              </div>
                  ...
                  <div key={i} className="ms-pl-it" data-widget={`recent.${i}`}>
                    ...
                    <div className="ms-pl-it-body">
                      <div className="ms-pl-it-t" data-field={`recent.${i}.t`} style={{ whiteSpace: 'pre-wrap' }}>
                        {pick(`recent.${i}.t` as keyof Required<MsPlaylistConfig>)}
                      </div>
                      <div className="ms-pl-it-s" data-field={`recent.${i}.s`} style={{ whiteSpace: 'pre-wrap' }}>
                        {pick(`recent.${i}.s` as keyof Required<MsPlaylistConfig>)}
                      </div>
                    </div>
                    <span className="ms-pl-it-when" data-field={`recent.${i}.w`} style={{ whiteSpace: 'pre-wrap' }}>
                      {pick(`recent.${i}.w` as keyof Required<MsPlaylistConfig>)}
                    </span>
                  </div>
                );
              })}
          ...
          <div className="ms-pl-stats" data-widget="stats">
            <div className="ms-pl-cell" data-widget="stats.attendance">
              <span className="ms-pl-cell-k" data-field="stats.attendance.k" style={{ whiteSpace: 'pre-wrap' }}>{pick('stats.attendance.k')}</span>
              <span className="ms-pl-cell-v ms-pl-green" data-field="stats.attendance.v" style={{ whiteSpace: 'pre-wrap' }}>{pick('stats.attendance.v')}</span>
            </div>
            <div className="ms-pl-dv" />
            <div className="ms-pl-cell" data-widget="stats.lunch">
              <span className="ms-pl-cell-k" data-field="stats.lunch.k" style={{ whiteSpace: 'pre-wrap' }}>{pick('stats.lunch.k')}</span>
              <span className="ms-pl-cell-v ms-pl-warm" data-field="stats.lunch.v" style={{ whiteSpace: 'pre-wrap' }}>{pick('stats.lunch.v')}</span>
            </div>
            <div className="ms-pl-dv" />
            <div className="ms-pl-cell" data-widget="stats.bus">
              <span className="ms-pl-cell-k" data-field="stats.bus.k" style={{ whiteSpace: 'pre-wrap' }}>{pick('stats.bus.k')}</span>
              <span className="ms-pl-cell-v" data-field="stats.bus.v" style={{ whiteSpace: 'pre-wrap' }}>{pick('stats.bus.v')}</span>
            </div>
          </div>
        </aside>
      </section>
      ...
      <section className="ms-pl-bottom">
        <div className="ms-pl-alert" data-widget="alert">
          <div className="ms-pl-alert-ico" data-field="alert.ico" style={{ whiteSpace: 'pre-wrap' }}>{pick('alert.ico')}</div>
          <div className="ms-pl-alert-body">
            <div className="ms-pl-alert-ey" data-field="alert.eyebrow" style={{ whiteSpace: 'pre-wrap' }}>{pick('alert.eyebrow')}</div>
            <div className="ms-pl-alert-msg" data-field="alert.message" style={{ whiteSpace: 'pre-wrap' }}>{pick('alert.message')}</div>
          </div>
          <div className="ms-pl-alert-when" data-field="alert.when" style={{ whiteSpace: 'pre-wrap' }}>{pick('alert.when')}</div>
        </div>

        <div className="ms-pl-ticker" data-widget="ticker">
          <div className="ms-pl-ticker-badge" data-field="ticker.tag" style={{ whiteSpace: 'pre-wrap' }}>{pick('ticker.tag')}</div>
