  ...
  return (
    <div style={frameStyle(r)}>
      <div style={{ textAlign: 'center', borderBottom: `2px solid ${r.accent.primary}`, paddingBottom: 8, marginBottom: 12 }}>
        ...
        <div style={{ color: r.accent.primary, fontSize: '1.6em', textShadow: `0 0 16px ${r.accent.primary}`, letterSpacing: '0.1em' }}>{c.title || 'EAT HERE'}</div>
      </div>
  ...
  return (
    <div style={frameStyle(r)}>
      <div style={{ textAlign: 'center', borderBottom: `2px dashed ${r.accent.primary}`, paddingBottom: 6, marginBottom: 10 }}>
        <h2 style={{ margin: 0, fontSize: '2em', fontFamily: "'Caveat', cursive", color: r.accent.primary, fontWeight: 700 }}>{c.title || "Today's Menu"}</h2>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 6, fontSize: r.font.size }}>
  ...
  return (
    <div style={frameStyle(r)}>
      <div style={{ textAlign: 'center' }}>
        <h2 style={{ margin: 0, fontSize: '1.6em', fontWeight: 800 }}>🍽️ {c.title || "Today's Lunch!"}</h2>
        <div style={{ fontSize: '0.85em', fontWeight: 700, color: r.accent.primary, letterSpacing: '0.2em' }}>YUMMY YUMMY!</div>
      </div>
      <div style={{ marginTop: 16, background: '#fff', border: `4px solid ${r.accent.secondary}`, borderRadius: 24, padding: 16, transform: 'rotate(-1deg)', boxShadow: '0 8px 0 rgba(0,0,0,0.1)' }}>
  ...
  return (
    <div style={{ ...frameStyle(r), backdropFilter: 'blur(20px)' }}>
      <div style={{ textAlign: 'center', borderBottom: `1px solid ${r.accent.primary}`, paddingBottom: 10, marginBottom: 14 }}>
        ...
        <h2 style={{ margin: '4px 0 0 0', fontSize: '2em', fontStyle: 'italic', fontWeight: 500 }}>{c.title || 'Cafeteria'}</h2>
      </div>
      <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 12, fontSize: r.font.size }}>
...
export function LunchOpsInventoryWidget({ config }: WidgetProps<LunchCfg>) {
  ...
  return (
    <div style={frameStyle(r)}>
      <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: `1px dashed ${r.accent.primary}55`, paddingBottom: 6, marginBottom: 8, fontSize: '1.1em' }}><b style={{ color: r.accent.primary, letterSpacing: '0.2em' }}>● MENU.WEEK_PLAN</b><span style={{ color: r.accent.secondary }}>{c.title || 'CAFE'}</span></div>
      <div style={{ display: 'grid', gridTemplateColumns: '60px 1fr 1fr auto', columnGap: 8, rowGap: 4, fontSize: r.font.size }}>
