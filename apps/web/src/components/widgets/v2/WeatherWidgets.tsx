    ...
    <div style={frameStyle(r)}>
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20, flex: 1 }}>
          <div style={{ fontSize: 80 }}>{c.staticIcon || emojiFor(w.code)}</div>
  ...
  return (
    <div style={frameStyle(r)}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginTop: 16 }}>
        <div style={{ fontSize: 80, filter: 'sepia(0.3)' }}>{c.staticIcon || emojiFor(w.code)}</div>
        <div><div style={{ fontSize: 84, fontWeight: 900, lineHeight: 1, fontFamily: 'monospace' }}>{temp}{u}</div><div style={{ fontStyle: 'italic', fontSize: '1.4em', color: r.accent.primary }}>{c.staticDesc || descFor(w.code)}</div></div>
      </div>
      <div style={{ marginTop: 14, borderTop: '1px dashed #94a3b8', paddingTop: 8, display: 'flex', justifyContent: 'space-between', fontSize: '0.95em', fontStyle: 'italic' }}><span>High: {w.hi}{u}</span><span>Low: {w.lo}{u}</span><span>{c.location || 'Local'}</span></div>
    </div>
  );
}
  ...
  return (
    <div style={frameStyle(r)}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 10 }}>
        <div style={{ fontSize: '0.9em', fontWeight: 800, color: r.accent.secondary, background: '#fff', padding: '6px 18px', borderRadius: 999, transform: 'rotate(-3deg)', boxShadow: '0 4px 0 rgba(0,0,0,0.1)' }}>📍 {c.location || 'Our School'}</div>
        <div style={{ width: 130, height: 130, background: r.accent.primary, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 70, boxShadow: '0 8px 0 rgba(0,0,0,0.12)' }}>{c.staticIcon || emojiFor(w.code)}</div>
        <div style={{ fontSize: 84, fontWeight: 800, lineHeight: 1, color: r.accent.secondary, textShadow: '4px 4px 0 #fff' }}>{temp}{u}</div>
        <div style={{ background: r.accent.highlight, color: '#fff', padding: '6px 18px', borderRadius: 999, fontWeight: 800, fontSize: '0.95em' }}>{c.staticDesc || descFor(w.code)}!</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ ...frameStyle(r), backdropFilter: 'blur(20px)' }}>
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 8 }}>
        <div style={{ fontSize: '0.85em', fontWeight: 600, letterSpacing: '0.2em', color: r.accent.primary, textTransform: 'uppercase' }}>{c.location || 'Local Weather'}</div>
        <div style={{ display: 'flex', alignItems: 'center', flex: 1, gap: 16 }}>
          <div style={{ fontSize: 88 }}>{c.staticIcon || emojiFor(w.code)}</div>
          <div><div style={{ fontSize: 90, fontWeight: 200, letterSpacing: '-0.04em', lineHeight: 1 }}>{temp}<span style={{ fontSize: '0.4em', color: '#64748b' }}>{u}</span></div><div style={{ fontSize: '1em', color: '#64748b', fontWeight: 500 }}>{c.staticDesc || descFor(w.code)}</div></div>
        </div>
    ...
    <div style={frameStyle(r)}>
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 6 }}>
        ...
        <div style={{ flex: 1, fontSize: r.font.size, lineHeight: 1.7, color: r.accent.secondary }}>
          ...
          <div><b style={{ color: r.font.color }}>ICON&nbsp;:</b> {c.staticIcon || emojiFor(w.code)} <span style={{ color: r.accent.primary }}>WMO_{w.code}</span></div>
        </div>
