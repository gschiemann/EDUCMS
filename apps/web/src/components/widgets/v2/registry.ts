...
export const HEALTHCARE_WIDGETS: RegisteredWidget[] = [
  ...
  // 2026-05-29 editability fix — added rows array default so the auto-form
  // generator in PropertiesPanel produces a comma-split text field for each
  // department row instead of showing zero content fields.
  W('WAIT_TIMES_BOARD',   CAT_HEALTHCARE, 'Wait Times Board',   'Per-department wait estimates, traffic-light', 'universal', Clock,         withMeasuredHeight(WaitTimesBoardWidget),    { rows: [{ dept: 'Emergency · Triage', note: 'Walk-in', queued: 4, wait: 12 }, { dept: 'Urgent Care', note: 'Level 4-5 conditions', queued: 8, wait: 38 }, { dept: 'Radiology · X-Ray', note: 'Walk-in + scheduled', queued: 3, wait: 22 }, { dept: 'Lab Draw', note: 'Outpatient', queued: 11, wait: 18 }, { dept: 'Pharmacy', note: 'Prescription pickup', queued: 6, wait: 9 }] }),
  // 2026-05-29 — added bio/specialties/years/patients/rating so ProviderSpotlight
  // exposes all editable content fields (previously only name+title were shown).
  W('PROVIDER_SPOTLIGHT', CAT_HEALTHCARE, 'Provider Spotlight', 'Doctor / staff hero card with bio',            'universal', UserCog,       withMeasuredHeight(ProviderSpotlightWidget), { name: 'Dr. Aisha Pereira', title: 'Cardiothoracic Surgeon · MD, FACS', bio: 'Board-certified surgeon with a focus on minimally-invasive procedures.', specialties: ['Coronary bypass', 'Valve replacement', 'Heart failure'], years: 16, patients: '2,400+', rating: '4.9' }),
  // 2026-05-29 — added points array + qrLabel so PatientEducation exposes
  // the full bullet list and QR footnote in the editor.
  W('PATIENT_EDUCATION',  CAT_HEALTHCARE, 'Patient Education',  'Waiting-room education with QR-to-phone',      'universal', BookOpen,      withMeasuredHeight(PatientEducationWidget),  { title: 'Managing high blood pressure', points: ['Aim for less than 1500 mg sodium per day', 'Daily walking, 30 minutes, lowers BP by ~5 mmHg', 'Take medications at the same time daily', 'Track your home readings — bring them to every visit'], qrLabel: 'Take this on your phone' }),
  // 2026-05-29 — added hours rows + note so VisitorHours shows its list editor.
  W('VISITOR_HOURS',      CAT_HEALTHCARE, 'Visitor Hours',      'Multi-unit visitor hours table',               'universal', CalendarClock, withMeasuredHeight(VisitorHoursWidget),      { note: 'Two visitors per patient. No children under 12 in ICU.', hours: [{ unit: 'General medical & surgical', note: 'Floors 3-6', hours: '8:00 AM – 9:00 PM' }, { unit: 'Maternity', note: '5th floor west', hours: '24 hours, partners only at night' }, { unit: 'ICU & Critical Care', note: '2nd floor, badge req.', hours: '10:00 AM – 8:00 PM' }, { unit: 'Pediatrics', note: '4th floor', hours: '9:00 AM – 8:00 PM' }] }),
  W('CODE_BANNER',        CAT_HEALTHCARE, 'Code Banner',        'Flashing overlay for hospital codes',          'universal', AlertOctagon,  withMeasuredHeight(CodeBannerWidget),        { code: 'CODE BLUE', location: '4-WEST · ROOM 412', kind: 'STAFF EMERGENCY ALERT' }),
  // 2026-05-29 — added carriers array so InsuranceAccepted shows its list editor.
  W('INSURANCE_ACCEPTED', CAT_HEALTHCARE, 'Insurance Accepted', 'Tile board of accepted insurance carriers',    'universal', ShieldCheck,   withMeasuredHeight(InsuranceAcceptedWidget), { carriers: [{ name: 'Aetna', color: '#7c3aed', note: 'Most plans' }, { name: 'Anthem Blue Cross', color: '#1d4ed8', note: 'PPO + HMO' }, { name: 'United Healthcare', color: '#0ea5e9', note: 'Most plans' }, { name: 'Cigna', color: '#15803d', note: 'Most plans' }, { name: 'Humana', color: '#16a34a', note: 'Medicare Advantage' }] }),
];

export const CORPORATE_WIDGETS: RegisteredWidget[] = [
  // 2026-05-29 — added events array to RoomSchedule so up-next rows are editable.
  W('ROOM_SCHEDULE',      CAT_CORPORATE, 'Meeting Room Schedule',  'Door-sign room status + up-next list',  'universal', CalendarRange, withMeasuredHeight(RoomScheduleWidget),      { room: 'Pacific · 12-A', status: 'AVAILABLE', events: [{ start: '2:30 PM', title: 'Q3 Engineering Planning', host: 'A. Chen', seats: 14 }, { start: '4:00 PM', title: 'Design review · Onboarding', host: 'M. Stevens', seats: 8 }] }),
  // 2026-05-29 — added where/meeting to VisitorWelcome for full editability.
  W('VISITOR_WELCOME',    CAT_CORPORATE, 'Visitor Welcome Board',  'Lobby greeter, personalized name',      'universal', UserCheck,     withMeasuredHeight(VisitorWelcomeWidget),    { host: 'Northwind HQ', visitor: 'Alex Morgan', company: 'Acme Robotics', meeting: 'Dana Stevens', where: 'Lobby reception' }),
  // 2026-05-29 — added trend sparkline array and suffix for full KPI edit.
  W('KPI_TILE',           CAT_CORPORATE, 'KPI Tile',               'One-metric tile + sparkline + delta',   'universal', TrendingUp,    withMeasuredHeight(KpiTileWidget),           { label: 'Revenue · MRR', value: '1.42M', prefix: '$', suffix: '', delta: 12, target: '1.5M', trend: [5, 8, 6, 9, 11, 9, 13, 15, 14, 17] }),
  // 2026-05-29 — added rows array so the leaderboard rows are editable.
  W('SALES_LEADERBOARD',  CAT_CORPORATE, 'Sales Leaderboard',      'Rep ranking with quota %',              'universal', Trophy,        withMeasuredHeight(SalesLeaderboardWidget),  { title: 'Sales · September', goal: '$2.5M', percent: 78, rows: [{ name: 'Priya Patel', team: 'West', deals: 12, amount: 412000, pct: 137, color: '#7c3aed' }, { name: 'Jordan Reyes', team: 'East', deals: 9, amount: 298000, pct: 119, color: '#0ea5e9' }, { name: 'Sam Whitaker', team: 'West', deals: 8, amount: 234000, pct: 104, color: '#22c55e' }] }),
  // 2026-05-29 — added room field to DoorSign so all copy is editable.
  W('DOOR_SIGN',          CAT_CORPORATE, 'Office Door Sign',       'Portrait office sign — availability',   'universal', DoorOpen,      withMeasuredHeight(DoorSignWidget),          { occupant: 'Dana Stevens', title: 'VP Engineering', status: 'AVAILABLE', room: 'Room 412' }),
  // 2026-05-29 — added okrs array so objectives/progress bars are editable.
  W('OKR_TRACKER',        CAT_CORPORATE, 'OKR Tracker',            'Objective + key-result progress bars',  'universal', Target,        withMeasuredHeight(OkrTrackerWidget),        { quarter: 'Q3', okrs: [{ title: 'Reach $2M ARR by end of Q3', pct: 78, krs: [{ label: 'New ARR', now: '$1.42M', target: '$2.0M' }] }, { title: 'NPS ≥ 70', pct: 62, krs: [{ label: 'NPS', now: 68, target: 70 }] }, { title: 'Ship multi-tenant SSO', pct: 35, krs: [{ label: 'Connectors', now: 2, target: 5 }] }] }),
  // 2026-05-29 — added list array so anniversary/birthday rows are editable.
  W('TEAM_ANNIVERSARIES', CAT_CORPORATE, 'Anniversaries & B-days', 'Work anniversaries + birthdays',        'universal', PartyPopper,   withMeasuredHeight(TeamAnniversariesWidget), { company: 'Northwind', list: [{ name: 'Priya Patel', team: 'Sales · West', years: 5, type: 'work', color: '#7c3aed' }, { name: 'Jordan Reyes', team: 'Customer Success', years: 3, type: 'work', color: '#0ea5e9' }, { name: 'Sam Whitaker', team: 'Engineering', years: 1, type: 'work', color: '#22c55e' }] }),
];

export const HOSPITALITY_WIDGETS: RegisteredWidget[] = [
  // 2026-05-29 — added room + checkin + checkout to HotelWelcome so all
  // personalisation fields appear in the editor.
  W('HOTEL_WELCOME',      CAT_HOSPITALITY, 'Hotel Guest Welcome', 'Editorial welcome card, personalized', 'universal', BedDouble, withMeasuredHeight(HotelWelcomeWidget),     { hotel: 'THE COPPERLEAF', guest: 'The Park family', room: 'Pinecrest 412', checkin: '3:18 PM', checkout: 'Sun, May 19' }),
  // 2026-05-29 — added events array to DailyEventsBoard so its rows are editable.
  W('DAILY_EVENTS_BOARD', CAT_HOSPITALITY, 'Daily Events Board',  "Today's activity + dining schedule",   'universal', Calendar,  withMeasuredHeight(DailyEventsBoardWidget), { property: 'THE COPPERLEAF', events: [{ time: '7:00 AM', category: 'Wellness', title: 'Mountain Sunrise Yoga', where: 'Garden Pavilion · Level 3', host: 'with Marisol', access: 'GUESTS' }, { time: '11:00 AM', category: 'Tasting', title: 'Coffee Cupping Experience', where: 'Lobby Café', host: 'with Roastmaster Daniel', access: 'OPEN' }, { time: '7:30 PM', category: 'Music', title: 'Live Jazz · Trio Maple', where: 'Library Bar', host: 'Reservations welcome', access: 'GUESTS' }] }),
  // 2026-05-29 — added amenities array so AmenityHours shows its list editor.
  W('AMENITY_HOURS',      CAT_HOSPITALITY, 'Amenity Hours Board', 'Pool / gym / spa open-closed status',  'universal', Clock4,    withMeasuredHeight(AmenityHoursWidget),     { amenities: [{ name: 'Heated Outdoor Pool', icon: '🏊', hours: '7 AM – 11 PM', status: 'OPEN' }, { name: 'Fitness Center', icon: '🏋️', hours: '24 hours', status: 'OPEN' }, { name: 'Frost Creek Spa', icon: '💆', hours: '9 AM – 9 PM', status: 'OPEN' }, { name: 'Business Center', icon: '💻', hours: '24 hours', status: 'OPEN' }] }),
  // 2026-05-29 — added all four text fields for CheckInOut widget.
  W('CHECK_IN_OUT_TIMES', CAT_HOSPITALITY, 'Check-in/out Times',  'Check-in / check-out windows',         'universal', Key,       withMeasuredHeight(CheckInOutTimesWidget),  { checkinTime: '3:00 PM', checkoutTime: '11:00 AM', checkinNote: 'Early check-in based on availability. Welcome drinks 4-6 PM in the lobby.', checkoutNote: 'Late check-out until 1 PM by request. Bag storage available.' }),
  // 2026-05-29 — added attractions array so the list rows are editable.
  W('LOCAL_ATTRACTIONS',  CAT_HOSPITALITY, 'Local Attractions',   'Curated nearby dining + activities',   'universal', MapPin,    withMeasuredHeight(LocalAttractionsWidget), { property: 'THE COPPERLEAF', attractions: [{ name: 'Vail Mountain', category: 'OUTDOORS', distance: '0.4 mi', travel: 'Free shuttle', rating: '4.9', price: '$$$' }, { name: 'Sweet Basil', category: 'DINING', distance: '0.6 mi', travel: '8 min walk', rating: '4.7', price: '$$$' }, { name: 'Eagle Bahn Gondola', category: 'OUTDOORS', distance: '0.3 mi', travel: '5 min walk', rating: '4.8', price: '$$' }, { name: 'Betty Ford Gardens', category: 'CULTURE', distance: '1.1 mi', travel: '3 min drive', rating: '4.8', price: 'Free' }] }),
];

export const WORSHIP_WIDGETS: RegisteredWidget[] = [
  // 2026-05-29 — added services array so the full schedule is editable.
  W('SERVICE_TIMES',      CAT_WORSHIP, 'Service Times',      'Weekly service schedule',                'universal', CalendarDays,   withMeasuredHeight(ServiceTimesWidget),      { label: 'Weekly Gatherings', services: [{ day: 'Sunday', subtitle: 'Traditional Service', where: 'Main Sanctuary', time: '8:30 AM' }, { day: 'Sunday', subtitle: 'Contemporary Service', where: 'Main Sanctuary', time: '10:30 AM' }, { day: 'Wednesday', subtitle: 'Midweek Prayer', where: 'Fellowship Hall', time: '7:00 PM' }] }),
  // 2026-05-29 — added title/reference/speaker/date so all sermon card
  // fields appear in the editor (previously only series was editable).
  W('SERMON_TITLE_CARD',  CAT_WORSHIP, 'Sermon Title Card',  'Editorial sermon hero — series + title', 'universal', Cross,          withMeasuredHeight(SermonTitleCardWidget),   { series: 'The Sermon on the Mount', title: 'Blessed are the peacemakers', reference: 'Matthew 5:9', speaker: 'Pastor Mara Lin', date: 'Sunday · May 18, 2026' }),
  // 2026-05-29 — added hymns array so HymnBoard list is editable.
  W('HYMN_BOARD',         CAT_WORSHIP, 'Hymn Board',         "Today's hymn numbers + titles",          'universal', Music,          withMeasuredHeight(HymnBoardWidget),         { hymns: [{ number: '14', title: 'Holy, Holy, Holy', composer: 'R. Heber & J. Dykes', verses: '1, 2, 4' }, { number: '142', title: 'Be Thou My Vision', composer: 'Irish trad., M. Byrne', verses: 'all' }, { number: '408', title: 'Take My Life and Let It Be', composer: 'F. R. Havergal', verses: '1-3' }, { number: '267', title: 'Great Is Thy Faithfulness', composer: 'T. Chisholm & W. Runyan', verses: 'all' }] }),
  // 2026-05-29 — added donors/daysLeft/qrLabel so thermometer is fully editable.
  W('GIVING_THERMOMETER', CAT_WORSHIP, 'Giving Thermometer', 'Campaign progress with QR-to-give',       'universal', HandCoins,      withMeasuredHeight(GivingThermometerWidget), { label: 'Capital Campaign', title: 'Build the new student wing', goal: 250000, raised: 167200, donors: 312, daysLeft: 47, qrLabel: 'connect.firstchurch.org/give' }),
  // 2026-05-29 — added text and translation so the verse body is editable.
  W('SCRIPTURE_VERSE',    CAT_WORSHIP, 'Scripture Verse',    'Centered editorial verse card',          'universal', BookOpen,       withMeasuredHeight(ScriptureVerseWidget),    { reference: 'John 3:16', translation: 'KJV', text: 'For God so loved the world, that he gave his only begotten Son, that whosoever believeth in him should not perish, but have everlasting life.' }),
  // 2026-05-29 — added body text so prayer request copy is editable.
  W('PRAYER_REQUEST_QR',  CAT_WORSHIP, 'Prayer Request QR',  'QR to a confidential prayer form',       'universal', HeartHandshake, withMeasuredHeight(PrayerRequestQrWidget),   { qrLabel: 'firstchurch.org/prayer', body: 'Scan with your phone to send a confidential request to our prayer team. Replies within 24 hours.' }),
];
