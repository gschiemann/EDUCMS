...
export function UsbIngestCard() {
  ...
  const handleToggle = async (next: boolean) => {
    if (next && !cfg?.hasKey) {
      ...
      const ok = await appConfirm({
        title: t('settings.usbCard.enableConfirmTitle'),
        message: t('settings.usbCard.enableConfirmMessage'),
        confirmLabel: t('settings.usbCard.enableConfirmLabel'),
      });
      if (!ok) return;
      toggle.mutate(true, {
        onSuccess: () => {
    ...
    if (!next) {
      const ok = await appConfirm({
        title: t('settings.usbCard.disableConfirmTitle'),
        message: t('settings.usbCard.disableConfirmMessage'),
        confirmLabel: t('settings.common.disable'),
        tone: 'danger',
      });
      if (!ok) return;
    }
  ...
  const handleRotate = async () => {
    const ok = await appConfirm({
      title: t('settings.usbCard.rotateConfirmTitle'),
      message: t('settings.usbCard.rotateConfirmMessage'),
      confirmLabel: t('settings.usbCard.rotateKey'),
      tone: 'danger',
    });
    if (!ok) return;
    rotate.mutate(undefined, {
  ...
  const copyKey = async () => {
    ...
    try {
      ...
      setCopyError(t('settings.usbCard.clipboardBlocked'));
    }
  };
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-100">
        <h2 className="text-sm font-bold text-slate-700 flex items-center gap-2">
          <Usb className="w-4 h-4 text-violet-500" /> {t('settings.usbCard.title')}
        </h2>
        <p className="text-xs text-slate-500 mt-1">
          {t('settings.usbCard.subtitle')}
        </p>
      </div>
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                <div className="text-sm font-bold text-slate-800 flex items-center gap-2">
                  ...
                  {cfg?.enabled ? t('settings.usbCard.stateEnabled') : t('settings.usbCard.stateDisabled')}
                </div>
                <p className="text-xs text-slate-500 mt-0.5">
                  {cfg?.enabled
                    ? t('settings.usbCard.stateEnabledHint')
                    : t('settings.usbCard.stateDisabledHint')}
                </p>
              </div>
              <button
                type="button"
              ...
              >
                {toggle.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : cfg?.enabled ? t('settings.common.disable') : t('settings.common.enable')}
              </button>
            </div>
            ...
            {cfg?.enabled && (
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                      <KeyRound className="w-3.5 h-3.5 text-violet-500" /> {t('settings.usbCard.hmacKeyTitle')}
                    </div>
                    <p className="text-[11px] text-slate-500 mt-0.5">
                      {cfg.hasKey
                        ? t('settings.usbCard.keyActive', { date: cfg.keyRotatedAt ? new Date(cfg.keyRotatedAt).toLocaleString() : t('settings.usbCard.unknownDate') })
                        : t('settings.usbCard.noKeyYet')}
                    </p>
                  </div>
                  <button
                    type="button"
                  ...
                  >
                    {rotate.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : (cfg.hasKey ? t('settings.usbCard.rotateKey') : t('settings.usbCard.generateKey'))}
                  </button>
                </div>
                <div className="flex items-start gap-2 text-[11px] text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                  ...
                  <span>
                    {t('settings.usbCard.rotateWarning')}
                  </span>
                </div>
              </div>
            )}
            ...
            {cfg?.enabled && events && events.length > 0 && (
              <div className="rounded-xl border border-slate-200 overflow-hidden">
                <div className="px-4 py-2.5 bg-slate-50 border-b border-slate-200">
                  <div className="text-xs font-bold text-slate-700 flex items-center gap-1.5">
                    <ClipboardList className="w-3.5 h-3.5 text-slate-500" /> {t('settings.usbCard.recentEvents', { count: events.length })}
                  </div>
                </div>
                      ...
                      <li key={e.id} className="px-4 py-2.5 flex items-center justify-between gap-3 text-xs">
                        <div className="min-w-0">
                          <div className="font-bold text-slate-700 truncate">
                            {e.bundleVersion || t('settings.usbCard.noVersion')} · {t('settings.usbCard.assetCount', { count: e.assetCount })}
                            {e.emergencyAssets && <span className="ml-1.5 text-rose-600">🛡️ {t('settings.usbCard.emergencyTag')}</span>}
                          </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 space-y-4">
            <div>
              <h3 className="text-lg font-extrabold text-slate-900 flex items-center gap-2">
                <KeyRound className="w-5 h-5 text-violet-600" /> {t('settings.usbCard.newKeyTitle')}
              </h3>
              <p className="text-xs text-rose-700 mt-1 font-medium flex items-start gap-1.5">
                <AlertTriangle className="w-3.5 h-3.5 mt-0.5 shrink-0" />
                {t('settings.usbCard.shownOnce')}
              </p>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {copied ? <><Check className="w-4 h-4" /> {t('settings.common.copied')}</> : <><Copy className="w-4 h-4" /> {t('settings.usbCard.copyToClipboard')}</>}
              </button>
              <button
                type="button"
            ...
            <p className="text-[11px] text-slate-500">
              {t.rich('settings.usbCard.storeHint', {
                code: (chunks) => <code className="bg-slate-100 px-1 py-0.5 rounded">{chunks}</code>,
              })}
            </p>
          </div>
        </div>
