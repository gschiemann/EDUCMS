...
function build(opts: { availability?: any; checkout?: jest.Mock } = {}) {
  ...
  const allowance = {
    purchaseAvailabilityFor: jest.fn(
      async () => opts.availability ?? { enabled: true },
    ),
    orgTenantIdFor: jest.fn(async () => 'org_A'),
    boards: jest.fn(async () => ({
      packs: [
        {
          id: 'pur_1',
          pack: 'standard',
...
describe('POST /billing/ai-packs/checkout', () => {
  it("opens a Stripe-hosted checkout for the SESSION's organisation — never one named in the body", async () => {
    ...
    const out = await controller.aiPackCheckout(req(), {
      pack: 'standard',
      tenantId: 'org_EVIL',
      orgTenantId: 'org_EVIL',
    } as any);
    ...
    expect(stripe.checkoutBoardPack).toHaveBeenCalledWith({
      ...
      cancelUrl:
        'https://app.example/lincoln/settings/billing?boards=cancelled',
    });
  });
  ...
  it.each([
    [
      'NO_PLATFORM_KEY',
      'AI runs on your own key — add it in Settings → AI provider.',
    ],
    [
      'OWN_KEY',
    ...
    [
      ...
      "Buying more boards isn't set up on this deployment yet.",
    ],
  ])(
    '%s → { enabled: false, reason, message } and Stripe is never called',
    async (reasonCode, reason) => {
      const { controller, stripe } = build({
        availability: { enabled: false, reasonCode, reason },
      });
      await expect(
        controller.aiPackCheckout(req(), { pack: 'standard' }),
      ).resolves.toEqual({
        enabled: false,
        reason: reasonCode,
        message: reason,
  ...
  it('with the REAL rule: no platform design key on this deploy → NO_PLATFORM_KEY (production today)', async () => {
    ...
    try {
      ...
      await expect(
        controller.aiPackCheckout(req(), { pack: 'starter' }),
      ).resolves.toMatchObject({ enabled: false, reason: 'NO_PLATFORM_KEY' });
      expect(stripe.checkoutBoardPack).not.toHaveBeenCalled();
...
describe('GET /billing/ai-packs', () => {
  it("the packs, whether they can be bought, and the session organisation's purchases with what is left", async () => {
    ...
    await expect(
      controller.aiPacks(req({ query: { tenantId: 'org_EVIL' } })),
    ).resolves.toEqual({
      enabled: true,
      packs: [
...
describe('roles', () => {
  it('only the billing roles may buy or see purchases (never CONTRIBUTOR, never a viewer)', () => {
    for (const handler of ['aiPacks', 'aiPackCheckout'] as const) {
      const roles: AppRole[] = Reflect.getMetadata(
        ROLES_KEY,
        BillingController.prototype[handler],
      );
