...
describe('SEC-009 — two-tenant × every-role isolation matrix', () => {
  ...
  const DOCUMENTED_GAPS: Record<string, string> = {
    ...
    'passkey.controller.ts': '2026-09-21: there is no caller-supplied tenant or user id anywhere in it. The five signed-in routes act on the JWT principal\'s OWN rows — every passkey query is `{ ..., userId: <principal> }`, including the two that take a passkey `:id` (rename, remove), where ownership is IN the WHERE clause so there is no check-then-write window; cross-user rename AND delete both answer 404 in auth/passkey.controller.spec.ts. The six public routes derive the user from a signed mfaToken or from the unique credential id of a verified WebAuthn assertion, then run the same loginEligibility gate as password login (auth/passkey.login.spec.ts). 2026-09-21 added the last two of those six — the forced-enrollment pair /mfa/required/passkey/{options,verify}; their loader `requiredEnrollmentUser` takes the user id ONLY from the verified mfaToken\'s `sub`, reads the tenant off that row, and refuses anything the shared assertEnrollmentRequired + loginEligibility gates reject (auth/passkey.required-enrollment.spec.ts). The tenant is always read off the resolved user row, never supplied.',
