...
export const SYSTEM_TEMPLATE_PRESETS: SystemPreset[] = [
  ...
  },
  {
    ...
    name: '🏃 Track Day',
    description: 'Elementary running-track lobby — top-down stadium view, widgets placed around the infield, curved lanes, finish-line ticker.',
    category: 'LOBBY',
    orientation: 'LANDSCAPE',
    schoolLevel: 'ELEMENTARY',
    screenWidth: 3840,
    ...
    bgColor: '#4FB06B',
    zones: [
      { name: 'Infield Welcome Banner', widgetType: 'TEXT',           x: 18, y: 18, width: 64, height: 14, zIndex: 4, sortOrder: 0, defaultConfig: { theme: 'track-day', content: 'LET\'S RUN!', subtitle: '~ welcome back, team ~' } },
      { name: 'Center Mascot',           widgetType: 'LOGO',           x: 45, y: 40, width: 10, height: 16, zIndex: 6, sortOrder: 1, defaultConfig: { theme: 'track-day' } },
      { name: 'Coach Whistle Weather',   widgetType: 'WEATHER',        x: 72, y: 36, width: 12, height: 16, zIndex: 3, sortOrder: 2, defaultConfig: { theme: 'track-day', location: 'Springfield', units: 'imperial' } },
      { name: 'Bleacher Clipboard',      widgetType: 'ANNOUNCEMENT',   x: 55, y: 4, width: 28, height: 28, zIndex: 10, sortOrder: 3, defaultConfig: { theme: 'track-day', message: 'Field Day is Friday! Wear your school colors.' } },
      { name: 'Finish Line Countdown',   widgetType: 'COUNTDOWN',      x: 86, y: 12, width: 12, height: 76, zIndex: 4, sortOrder: 4, defaultConfig: { theme: 'track-day', label: 'Field Day in', targetDate: '' } },
      { name: 'Stopwatch',               widgetType: 'CLOCK',          x: 2, y: 72, width: 14, height: 22, zIndex: 5, sortOrder: 5, defaultConfig: { theme: 'track-day', format: '12h' } },
      { name: 'Lane Events',             widgetType: 'CALENDAR',       x: 18, y: 56, width: 40, height: 32, zIndex: 3, sortOrder: 6, defaultConfig: { theme: 'track-day', daysToShow: 7, maxEvents: 4 } },
      { name: 'Coach Polaroid',          widgetType: 'STAFF_SPOTLIGHT',x: 60, y: 60, width: 22, height: 28, zIndex: 3, sortOrder: 7, defaultConfig: { theme: 'track-day', staffName: 'Coach Martinez', role: 'Coach of the Month', bio: 'Making everyone their personal best.' } },
      { name: 'Finish Line Ticker',      widgetType: 'TICKER',         x: 0, y: 92, width: 100, height: 8, zIndex: 6, sortOrder: 8, defaultConfig: { theme: 'track-day', speed: 'medium', messages: ['Go team! ⚡', 'Track Day Saturday at 10am', 'Cheer section needed — all parents welcome', 'Bring water + sunscreen 🌞'] } },
    ],
  },
  {
    id: 'preset-lobby-scorebug',
    name: '📊 Scorebug Dashboard',
    description: 'Middle school sports-app dashboard. Matchup hero banner, live-standings, athlete-of-the-week, upcoming-games feed, and breaking-news chyron. ESPN energy for the lobby.',
    category: 'LOBBY',
    orientation: 'LANDSCAPE',
    schoolLevel: 'MIDDLE',
    screenWidth: 3840,
    ...
    bgColor: '#0B111C',
    zones: [
      { name: 'Live Ticker Strip',    widgetType: 'TICKER',         x: 0, y: 0, width: 100, height: 5, zIndex: 7, sortOrder: 0, defaultConfig: { theme: 'scorebug', speed: 'fast', messages: ['LIVE: Varsity football 14-7 Q3', 'Next home game: Friday 7PM vs Central', 'Soccer tryouts Thursday after school'] } },
      { name: 'Network Logo Bug',      widgetType: 'LOGO',           x: 2, y: 8, width: 8, height: 10, zIndex: 6, sortOrder: 1, defaultConfig: { theme: 'scorebug' } },
      { name: 'Matchup Hero Banner',   widgetType: 'TEXT',           x: 12, y: 7, width: 86, height: 20, zIndex: 4, sortOrder: 2, defaultConfig: { theme: 'scorebug', content: 'EAGLES vs COUGARS', subtitle: 'HOMECOMING · FRIDAY 7PM · HOME FIELD' } },
      { name: 'League Standings',      widgetType: 'CALENDAR',       x: 2, y: 30, width: 38, height: 52, zIndex: 3, sortOrder: 3, defaultConfig: { theme: 'scorebug', daysToShow: 7, maxEvents: 5 } },
      { name: 'Athlete of the Week',   widgetType: 'STAFF_SPOTLIGHT',x: 42, y: 30, width: 30, height: 52, zIndex: 3, sortOrder: 4, defaultConfig: { theme: 'scorebug', staffName: 'Jordan Miller', role: 'Point Guard · Grade 8', bio: 'Season avg: 18.4 PPG, 6 assists, 3 steals.' } },
      { name: 'Upcoming Games Feed',   widgetType: 'IMAGE_CAROUSEL', x: 74, y: 30, width: 24, height: 52, zIndex: 3, sortOrder: 5, defaultConfig: { theme: 'scorebug' } },
      { name: 'Status Bar (Clock)',    widgetType: 'CLOCK',          x: 2, y: 84, width: 14, height: 12, zIndex: 5, sortOrder: 6, defaultConfig: { theme: 'scorebug', format: '12h' } },
      { name: 'Status Bar (Weather)',  widgetType: 'WEATHER',        x: 17, y: 84, width: 18, height: 12, zIndex: 5, sortOrder: 7, defaultConfig: { theme: 'scorebug', location: 'Springfield', units: 'imperial' } },
      { name: 'T-Minus Countdown',     widgetType: 'COUNTDOWN',      x: 36, y: 84, width: 30, height: 12, zIndex: 5, sortOrder: 8, defaultConfig: { theme: 'scorebug', label: 'Homecoming in', targetDate: '' } },
      { name: 'Breaking News',         widgetType: 'ANNOUNCEMENT',   x: 67, y: 84, width: 31, height: 12, zIndex: 10, sortOrder: 9, defaultConfig: { theme: 'scorebug', message: 'Pep rally moved to Friday at 2:30 PM in the gym.' } },
    ],
  },

  // ─────────────────────────────────────────────────────
  // 🏟️ JUMBOTRON PRO — high-school stadium-scoreboard lobby
  // Massive center matchup scorebug, vertical stats rails on both
  // sides, sponsored corner panels, full-width LED ticker.
  // ─────────────────────────────────────────────────────
  {
    id: 'preset-lobby-jumbotron-pro',
    name: '🏟️ Jumbotron Pro',
    description: 'High school stadium jumbotron. Massive center scorebug, season-standings + player-stats rails, sponsored corner panels, full-width LED ticker. Friday-night-lights for the main lobby.',
    category: 'LOBBY',
    orientation: 'LANDSCAPE',
    schoolLevel: 'HIGH',
    screenWidth: 3840,
    ...
    bgColor: '#050A14',
    zones: [
      { name: 'Next Game Countdown',  widgetType: 'COUNTDOWN',      x: 2,  y: 3,  width: 20, height: 18, zIndex: 5,  sortOrder: 0, defaultConfig: { theme: 'jumbotron-pro', label: 'Kickoff in', targetDate: '' } },
      { name: 'Weather Chip',          widgetType: 'WEATHER',        x: 78, y: 3,  width: 20, height: 18, zIndex: 5,  sortOrder: 1, defaultConfig: { theme: 'jumbotron-pro', location: 'Springfield', units: 'imperial' } },
      { name: 'Stadium Bug',           widgetType: 'LOGO',           x: 45, y: 4,  width: 10, height: 10, zIndex: 8,  sortOrder: 2, defaultConfig: { theme: 'jumbotron-pro' } },
      { name: 'Season Standings',      widgetType: 'CALENDAR',       x: 2,  y: 23, width: 14, height: 66, zIndex: 3,  sortOrder: 3, defaultConfig: { theme: 'jumbotron-pro', daysToShow: 7, maxEvents: 6 } },
      { name: 'Player Stats',          widgetType: 'STAFF_SPOTLIGHT',x: 84, y: 23, width: 14, height: 66, zIndex: 3,  sortOrder: 4, defaultConfig: { theme: 'jumbotron-pro', staffName: 'Alex Rivera', role: 'QB #7 · Senior', bio: '32 TDs · 4,112 yds · 68% comp' } },
      { name: 'Center Scorebug',       widgetType: 'TEXT',           x: 18, y: 22, width: 64, height: 44, zIndex: 4,  sortOrder: 5, defaultConfig: { theme: 'jumbotron-pro', content: 'EAGLES   vs   COUGARS', subtitle: 'HOMECOMING · FRI 7PM · HOME FIELD' } },
      { name: 'Game Clock',            widgetType: 'CLOCK',          x: 18, y: 68, width: 22, height: 22, zIndex: 5,  sortOrder: 6, defaultConfig: { theme: 'jumbotron-pro', format: '12h' } },
      { name: 'Coach Message',         widgetType: 'ANNOUNCEMENT',   x: 41, y: 68, width: 38, height: 22, zIndex: 10, sortOrder: 7, defaultConfig: { theme: 'jumbotron-pro', message: 'GO EAGLES. Play smart, play hard, play together.' } },
      { name: 'Replay Panel',          widgetType: 'IMAGE_CAROUSEL', x: 80, y: 68, width: 18, height: 22, zIndex: 3,  sortOrder: 8, defaultConfig: { theme: 'jumbotron-pro' } },
      { name: 'LED Ticker',            widgetType: 'TICKER',         x: 0,  y: 91, width: 100, height: 9, zIndex: 7,  sortOrder: 9, defaultConfig: { theme: 'jumbotron-pro', speed: 'medium', messages: ['FINAL: Varsity 28 — Central 14', 'JV soccer moves on to regionals', 'Girls volleyball states next Saturday', 'Pep rally Friday @ 2:30 in the gym'] } },
    ],
  },
];
