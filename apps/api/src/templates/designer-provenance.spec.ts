...
import { TemplatesController, DesignerGenerateSchema, DesignerJobStartSchema } from './templates.controller';
import { DesignerJobsService } from './designer-jobs/designer-jobs.service';
import { designerOptionsFromJobRequest, replayableRequest, type DesignerGenerateOutput } from './designer-jobs/designer-job-request';
...
function build() {
  ...
  (controller as any).designerJobs = new DesignerJobsService({ client: db.client } as any);
  return { db, controller, syncCalls };
}

const LOGO = 'https://sb.example/storage/v1/object/public/assets/ai-designer/t-alpha/uploads/0123456789abcdef.png';
const PHOTO = 'https://sb.example/storage/v1/object/public/assets/ai-stock/t-alpha/fedcba9876543210.jpg';

const BODIES: Array<[string, Record<string, unknown>]> = [
  ['an uploaded logo + a stock photo', { prompt: 'a welcome board', logoUrl: LOGO, logoSource: 'upload', heroImageUrl: PHOTO, heroImageSource: 'stock' }],
  ['a site logo + a POS photo', { prompt: 'menu', logoUrl: LOGO, logoSource: 'site', heroImageUrl: PHOTO, heroImageSource: 'pos', purpose: 'menu' }],
  ['a site photo only', { prompt: 'promo', heroImageUrl: PHOTO, heroImageSource: 'site' }],
];
...
describe('the sync endpoint forwards provenance to AiService', () => {
  it.each(BODIES)('%s', async (_name, raw) => {
    ...
    await controller.generateDesignerCandidates(req, DesignerGenerateSchema.parse(raw) as any);
  ...
  it('negative control: an unknown source is refused at the boundary (never reaches a prompt)', () => {
    expect(DesignerGenerateSchema.safeParse({ prompt: 'x', heroImageSource: 'ai' }).success).toBe(false);
    expect(DesignerGenerateSchema.safeParse({ prompt: 'x', logoSource: 'pos' }).success).toBe(false);
  });
});
...
describe('a job runs the same provenance the sync endpoint would (parity oracle)', () => {
  ...
  it('a server-side Regenerate replays the stored provenance', async () => {
    ...
    const first = await controller.startDesignerJob(req, DesignerJobStartSchema.parse(raw));
    await db.client.aiDesignerJob.update({ where: { id: first.jobId }, data: { status: 'done' } });
    const again = await controller.againDesignerJob(req, first.jobId, undefined);
    const rows = (await db.client.aiDesignerJob.findMany({})) as any[];
    const replayed = rows.find((r) => r.id === again.jobId);
    expect(replayed.request).toMatchObject({ logoSource: 'upload', heroImageSource: 'stock', logoUrl: LOGO, heroImageUrl: PHOTO });
    const stored = replayableRequest(replayed.request);
    expect(stored.ok && DesignerGenerateSchema.parse(stored.body)).toMatchObject({ logoSource: 'upload', heroImageSource: 'stock' });
  });
});
