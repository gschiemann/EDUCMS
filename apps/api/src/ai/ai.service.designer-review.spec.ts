...
import { DesignerGenerationCancelled, type DesignerProgress } from './designer-generation-hooks';
import { DESIGNER_PER_CANDIDATE_MARKER } from './designer-prompt';

const dispatchMock = dispatchAi as unknown as jest.Mock;
const fixture = (name: string) => fs.readFileSync(path.join(__dirname, '__fixtures__', 'renderer', `${name}.response.json`), 'utf8');
...
const DRAFT = doc('Fresh roasted every morning, all day long, right here downtown.');
const REVISED = doc('REVISED — fresh roasted every morning, all day long, right here downtown.');

type Script = { draw?: (attempt: number, input: any) => any; critic?: () => string; revise?: () => any };
function scriptModel(s: Script = {}) {
  ...
  dispatchMock.mockImplementation(async (_p: any, input: any) => {
    ...
    if (input.system === DESIGNER_CRITIQUE_SYSTEM_PROMPT) {
      return { raw: s.critic ? s.critic() : JSON.stringify({ verdict: 'revise', score: 60, defects: [{ severity: 'major', where: 'subhead', what: 'too long', fix: 'shorten' }] }), model: 'gpt-6-sol', usage: { inputTokens: 3000, outputTokens: 400 }, truncated: false };
    }
    if (String(input.userPrompt).includes('You are REVISING an existing signage board')) {
      return s.revise ? s.revise() : { raw: REVISED, model: 'gpt-6-sol', usage: { inputTokens: 20000, outputTokens: 9000 }, truncated: false };
    }
    draws += 1;
    return s.draw ? s.draw(draws, input) : { raw: DRAFT, model: 'gpt-6-sol', usage: { inputTokens: 14000, outputTokens: 9000 }, truncated: false };
  });
}

/** The renderer answers by what it is shown: the revision one way, the draft another. */
function renderer(opts: { draft?: string; revised?: string; status?: number } = {}) {
  const posts: string[] = [];
  const fetchMock = jest.fn(async (_url: string, init: any) => {
    const html = JSON.parse(init.body).html as string;
    posts.push(html);
    if (opts.status) {
      return { ok: false, status: opts.status, json: async () => ({ contractVersion: 1, error: 'render_timeout', message: 'x' }) } as any;
    }
    const body = html.includes('REVISED') ? (opts.revised ?? RENDER_GOOD) : (opts.draft ?? RENDER_CROWDED);
    return { ok: true, status: 200, json: async () => JSON.parse(body) } as any;
  });
  const safeFetchMock = jest.fn(async (url: string) => ({ body: PNG, contentType: 'image/png', finalUrl: url, status: 200 }));
  const client = new DesignerRendererClient({
    fetch: fetchMock as any,
    safeFetch: safeFetchMock as any,
    env: { RENDERER_URL: 'http://renderer.railway.internal:8080', SUPABASE_URL: SUPA },
    log: () => {},
  });
...
function buildService(client: DesignerRendererClient, adds: string[] = []) {
  const prisma: any = {
    client: {
      tenant: {
        findUnique: jest.fn(async ({ where }: any) => (where.id === 't1' ? { id: 't1', name: 'T1', parentId: null, aiProvider: null, aiKeyEncrypted: null, aiModel: null } : null)),
        update: jest.fn(async () => ({})),
      },
      tenantBranding: { findUnique: jest.fn(async () => null) },
      template: { findMany: jest.fn(async () => []) },
      auditLog: { create: jest.fn(async ({ data }: any) => { auditRows.push(data); return data; }), findMany: jest.fn(async () => []) },
    },
  };
  const redis: any = {
    publisher: {
  ...
  const menu: any = { resolveMenuForLocation: jest.fn(async () => ({ items: [] })), resolvePosMenuForLocation: jest.fn(async () => ({ items: [] })) };
  const stock: any = { isConfigured: () => false, search: async () => null };
  return new AiService(prisma, redis, {} as any, { analyzeDesignReference: jest.fn() } as any, stock, menu, undefined, undefined, client);
}

// The fixtures were rendered at 3840 × 2160, so the boards are judged on that canvas's floor (52 px).
const OPTS = { tenantId: 't1', prompt: 'a coffee menu board', vertical: 'restaurant', venueName: 'Chrome Coffee', purpose: 'menu', screenWidth: 3840, screenHeight: 2160 };
const calls = (feature?: string) =>
  dispatchMock.mock.calls.filter(([, input]) => {
    ...
    const isRevise = String(input.userPrompt).includes('You are REVISING an existing signage board');
...
describe('the look-and-fix loop', () => {
  it('a board with a measured blocker is critiqued, revised once, re-rendered — and the better board ships', async () => {
    ...
    const adds: string[] = [];
    scriptModel();
    const out = await buildService(r.client, adds).generateDesignerBoardCandidates(OPTS);
    ...
    for (const [, input] of [...calls('critique'), ...calls('revise')]) {
      expect(input.images).toEqual([expect.objectContaining({ mediaType: 'image/webp', detail: 'high' })]);
    ...
    expect(r.posts.every((p) => p.includes('/*VOS-STAGE-SCALE*/') && p.includes('data-vos-csp'))).toBe(true);
    ...
    expect(audit.boards[0].review.metricsBefore).toMatchObject({ clipped: 1, emptyRatio: 0.625 });
  ...
  it('a revision that measures WORSE loses — the original ships', async () => {
    ...
    const out = await buildService(r.client).generateDesignerBoardCandidates(OPTS);
    for (const c of out.candidates) {
      expect(c.html).not.toContain('REVISED');
      expect(c.review).toEqual({ reviewed: true, revised: false, score: 90 });
    }
    expect(lastAudit().boards[0].review).toMatchObject({ scoreBefore: 90, scoreAfter: 52, verdictAfter: 'original-kept' });
  });

  it('a revision cut off mid-document fails the sanitizer — the original ships, never fewer boards', async () => {
    const r = renderer();
    scriptModel({ revise: () => ({ raw: REVISED.slice(0, REVISED.indexOf('</body>')), truncated: true, usage: { inputTokens: 1, outputTokens: 40000 } }) });
    const out = await buildService(r.client).generateDesignerBoardCandidates(OPTS);
    expect(out.candidates).toHaveLength(3);
    for (const c of out.candidates) expect(c.review).toEqual({ reviewed: true, revised: false, score: 75 });
  ...
  it('a passing critique and no measured blocker: no revise at all', async () => {
    ...
    scriptModel({ critic: () => '```json\n{"verdict":"pass","score":92,"defects":[]}\n```' });
    const out = await buildService(r.client).generateDesignerBoardCandidates(OPTS);
    expect(calls('revise')).toHaveLength(0);
    for (const c of out.candidates) expect(c.review).toEqual({ reviewed: true, revised: false, score: 90 });
  });
  ...
  it('a renderer outage (504) skips the review — the boards still ship, unreviewed', async () => {
    ...
    expect(lastAudit().boards[0].review.reason).toBe('render: HTTP 504 render_timeout');
  });
  ...
  it('progress: per candidate drawing → binding → rendering → reviewing → revising → rendering → done', async () => {
    ...
    await buildService(r.client).generateDesignerBoardCandidates({ ...OPTS, count: 1 }, { onProgress: (p) => events.push(p) });
    expect(events.map((e) => `${e.stage}${e.candidate ? `#${e.candidate}` : ''}`)).toEqual([
      'drawing',
      'drawing#1',
      'binding#1',
      'rendering#1',
  ...
  it('PROMPT CACHE: one reference set, and every draw and the revise share a byte-identical prefix', async () => {
    ...
    const prefix = String(draws[0].userPrompt).slice(0, String(draws[0].userPrompt).indexOf(DESIGNER_PER_CANDIDATE_MARKER));
    expect(prefix.startsWith('REFERENCE BOARDS')).toBe(true);
    expect(prefix.length).toBeGreaterThan(10_000);
    for (const i of draws) {
      expect(i.userPrompt.startsWith(prefix + DESIGNER_PER_CANDIDATE_MARKER)).toBe(true);
      expect(i.system).toBe(draws[0].system);
    }
    expect(revise.userPrompt.startsWith(prefix + '\n\nYou are REVISING')).toBe(true);
...
describe('the static checks (renderer on or off)', () => {
  it('a draft cut off at the output ceiling is redrawn ONCE, told so', async () => {
    ...
    scriptModel({
      draw: (n) =>
        n === 1
          ? { raw: DRAFT.slice(0, DRAFT.indexOf('</body>')), truncated: true, usage: { inputTokens: 1, outputTokens: 40000 } }
          : { raw: DRAFT, truncated: false, usage: { inputTokens: 1, outputTokens: 9000 } },
    });
    const out = await buildService(off).generateDesignerBoardCandidates({ ...OPTS, count: 1 });
    expect(out.candidates).toHaveLength(1);
    expect(out.candidates[0].review).toBeUndefined(); // renderer off
    const prompts = calls('draw').map(([, i]) => String(i.userPrompt));
    expect(prompts).toHaveLength(2);
    expect(prompts[1]).toContain('IMPORTANT — your previous answer was cut off after 40000 tokens before the document ended');
    expect(lastAudit()).toMatchObject({ staticRedraws: 1, reviewEnabled: false });
    expect(lastAudit().boards[0]).toMatchObject({ redrawn: true, truncated: false });
  });

  it('cut off twice → that board is dropped (never half a board)', async () => {
    const off = new DesignerRendererClient({ env: {} });
    scriptModel({ draw: () => ({ raw: DRAFT.slice(0, DRAFT.indexOf('</body>')), truncated: true }) });
    await expect(buildService(off).generateDesignerBoardCandidates({ ...OPTS, count: 1 })).rejects.toBeDefined();
  });
});
...
describe('draw-time vision', () => {
  it('a logo on OUR storage rides the draw as an image part (high detail), and the prompt says so', async () => {
    ...
    await buildService(r.client).generateDesignerBoardCandidates({ ...OPTS, logoUrl: LOGO });
    for (const [, i] of calls('draw')) {
      expect(i.images).toEqual([{ mediaType: 'image/png', base64: PNG.toString('base64'), detail: 'high' }]);
      expect(i.userPrompt).toContain('The logo above is attached as an image so you can see it');
    }
  ...
  it('a logo anywhere else is never fetched and nothing is attached', async () => {
    const r = renderer({ draft: RENDER_GOOD });
    scriptModel({ critic: () => '{"verdict":"pass","score":90,"defects":[]}' });
    await buildService(r.client).generateDesignerBoardCandidates({ ...OPTS, logoUrl: 'https://www.venue.example/logo.png' });
