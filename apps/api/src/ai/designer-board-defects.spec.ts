...
describe('documentIncompleteness', () => {
  ...
  it('<html> with no doctype, cut: still judged', () => {
    expect(documentIncompleteness('<html><body><div>x</div>')).toMatch(
      /<\/html>/,
    );
  });
  it('closes </html> but never closes its <body>: incomplete', () => {
    expect(
      documentIncompleteness('<!doctype html><html><body><div>x</div></html>'),
    ).toMatch(/<body>/);
  });
  it('a bare fragment is not judged here', () => {
    expect(
      documentIncompleteness('<div class="stage"><h1>Hi</h1></div>'),
    ).toBeNull();
  });
});
...
describe('textEditability', () => {
  ...
  it('aria-hidden ornament is not counted', () => {
    const t = textEditability(
      '<body><div aria-hidden="true">BIG S</div><h1 data-field="h">Hello</h1></body>',
    );
...
describe('designerBoardDefects', () => {
  it('a clean, whole board has no blocker (only the card numbers, as a minor)', () => {
    ...
    expect(d).toEqual([
      expect.objectContaining({ code: 'no-editable-text', severity: 'minor' }),
    ]);
  });
  ...
  it('TRUNCATED — the vendor flag alone is a blocker, with the token count', () => {
    ...
    expect(d[0]).toEqual({
      ...
      severity: 'blocker',
      detail:
        'the answer was cut off at the output ceiling after 40000 output tokens',
    });
  });

  it('INCOMPLETE — half a board is a blocker, and nothing else is judged on it', () => {
    const d = designerBoardDefects(CUT, {
      truncated: true,
      outputTokens: 12_000,
  ...
  it('a quarter or more of the copy uneditable is a major, never a blocker', () => {
    const html =
      '<!doctype html><html><body><div class="stage"><h1 data-field="headline">Lunch, Handled</h1><div class="badge"><div>DEAL</div><div>2 for 6</div><div>All Day</div></div></div></body></html>';
    const d = designerBoardDefects(html, {});
    expect(d).toEqual([
      expect.objectContaining({ code: 'no-editable-text', severity: 'major' }),
    ]);
    expect(hasBlocker(d)).toBe(false);
  });
  ...
  it('NO IMAGE SLOT — a supplied logo / photo the board does not use is a major', () => {
    const logo =
      'https://abc.supabase.co/storage/v1/object/public/assets/ai-designer/t1/logo-1.png';
    const hero =
      'https://abc.supabase.co/storage/v1/object/public/assets/ai-designer/t1/photo-2.jpg?w=1&h=2';
    const d = designerBoardDefects(BOARD, {
      logoUrl: logo,
      heroImageUrl: hero,
    });
    expect(
      d.filter((x) => x.code === 'no-image-slot').map((x) => x.severity),
    ).toEqual(['major', 'major']);
    // Used (attribute-escaped `&amp;` counts) → no defect.
    const withBoth = BOARD.replace(
      '<body>',
      `<body><img data-imgslot="logo" src="${logo}"><img data-imgslot="hero" src="${hero.replace(/&/g, '&amp;')}">`,
    );
    expect(
      designerBoardDefects(withBoth, {
        logoUrl: logo,
        heroImageUrl: hero,
      }).filter((x) => x.code === 'no-image-slot'),
    ).toEqual([]);
  });

  it("BINDING — the POS binder's verdict rides along as a blocker naming the rows", () => {
    const d = designerBoardDefects(BOARD, {
      binding: { ok: false, missing: [2, 5] },
    });
    expect(d[0]).toEqual({
      code: 'binding-incomplete',
      severity: 'blocker',
      detail: 'planned POS rows missing: [item.2], [item.5]',
    });
    expect(
      designerBoardDefects(BOARD, { binding: { ok: true, missing: [] } }).some(
        (x) => x.code === 'binding-incomplete',
      ),
    ).toBe(false);
  });
});
...
describe('staticRedrawNudge', () => {
  it('a cut-off draft is told it was cut off, after how many tokens, and what to do', () => {
    const n = staticRedrawNudge(
      designerBoardDefects(CUT, { truncated: true, outputTokens: 12_000 }),
      12_000,
    );
    expect(n).toBe(
  ...
  it('an uneditable draft is told every text element needs data-field', () => {
    const n = staticRedrawNudge(
      designerBoardDefects(BOARD.replace(/ data-field="[^"]*"/g, ''), {}),
    );
  ...
  it('nothing to say for a board with only majors/minors (no redraw)', () => {
    expect(
      staticRedrawNudge(
        designerBoardDefects(BOARD, { logoUrl: 'https://x/l.png' }),
      ),
    ).toBe('');
  });
});
