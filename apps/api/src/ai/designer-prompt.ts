...
const GOLD_STANDARD_CRAFT: string[] = [
  `GOLD-STANDARD CRAFT BAR (the operator's explicit benchmark: every AI board must hit the CRAFT LEVEL of the finalized Rainbow-Animated template — every key element a DESIGNED OBJECT with silhouette, volume, and character — adapted to the brand's mood. The bar is the CRAFT, not the rainbow style, and NOT motion: animation is OPT-IN (see the motion rule below) — a still board can fully hit this bar. Obey alongside, never above, TYPOGRAPHY-IS-PRIORITY-ONE and the NO-REDACTION-BARS law):`,
  ...
  `- MOTION IS OPT-IN — OFF BY DEFAULT. Do NOT add ANY looping animation unless the operator's brief EXPLICITLY asks for movement (words like animated, motion, dynamic, lively, moving, spinning, celebrate, energy, kinetic, "make it move"). The DEFAULT board is completely STILL, and a still board is NOT "a flat slide" — its craft comes from objecthood, depth, type, and color, never from movement. NEVER add a spinning ray-halo, a breathing/bouncing badge, twinkling sparkles, drifting confetti, a sheen sweep, or any ambient animation to a board whose brief did not ask for it (that exact over-animation — the same spinning motif on every board — is the failure we are fixing). ONLY when the brief explicitly requests motion: add it tastefully per the MOTION CONTRACT below (transform/opacity only, on decoration / well-margined objects, energy dialed to the mood).`,
  ...
  `PER-VERTICAL MOOD MAP (the craft is constant; the FORM, palette + type are mood-specific). NOTE: motion is OPT-IN (see the motion rule above) — each mood's "Motion …" note is ONLY the energy to use IF the brief explicitly asked for movement; with no such request, ignore it and keep the board still:`,
