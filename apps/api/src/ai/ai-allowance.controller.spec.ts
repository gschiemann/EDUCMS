...
function build() {
  const allowance = {
    operatorView: jest.fn(async (tenantId: string, opts: any) => ({
      tenantId,
      opts,
    })),
  };
...
describe('AiAllowanceController', () => {
  it('reads the tenant from the SESSION — a tenant id in the query or body is never consulted', async () => {
    ...
    expect(allowance.operatorView).toHaveBeenCalledWith('t-home', {
      includeCost: false,
    });
  });
  ...
  it('our cost per board rides along for SUPER_ADMIN only', async () => {
    ...
    for (const role of [
      ...
      expect(allowance.operatorView).toHaveBeenLastCalledWith('t1', {
        includeCost: false,
      });
    }
    await controller.allowanceForSession({
      user: { tenantId: 't1', role: AppRole.SUPER_ADMIN },
    });
    expect(allowance.operatorView).toHaveBeenLastCalledWith('t1', {
      includeCost: true,
    });
  ...
  it('every role that can make or edit a board may read it', () => {
    const roles: AppRole[] = Reflect.getMetadata(
      ROLES_KEY,
      AiAllowanceController.prototype.allowanceForSession,
    );
