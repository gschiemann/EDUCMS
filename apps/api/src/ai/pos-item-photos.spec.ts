...
const OUR_ITEM_URL =
  /^https:\/\/sb\.example\/storage\/v1\/object\/public\/assets\/ai-designer\/t1\/item-[0-9a-f]{16}\.jpg$/;
const SELECTION = {
...
describe('the producer chain carries the POS photo to the plan (and nothing else does)', () => {
  it('ResolvedMenu items hold the https photo Toast gave; an http one never survived the sync', async () => {
    ...
    const resolved = await menu.resolvePosMenuForLocation('t1', {
      connectionId: TOAST_CONNECTION_ID,
      includeUnavailable: true,
      ignoreDayparts: true,
    });
    const byName = new Map(resolved.items.map((i) => [i.name, i.imageUrl]));
    expect(byName.get('3 Birria Tacos w/ consome')).toBe(
...
describe('loadPosBindingPlan — item photos checked and copied BEFORE the draw', () => {
  it('each row gets OUR copy of its own POS photo, or none; the POS URL never reaches the plan output', async () => {
    ...
    const plan = await loadPosBindingPlan(
      { prisma, menu, photos: { storage: bucket, fetch: cdn.fetch } },
      't1',
      SELECTION,
      CANVAS,
    );
    ...
    expect(bucket.uploads.map((u) => u.contentType)).toEqual([
      'image/jpeg',
      'image/jpeg',
    ]);
    expect(
    ...
    expect(lines.filter((l) => /^item\.\d+\.photo: /.test(l))).toEqual(
      plan.items
        .filter((i) => i.imageUrl)
        .map((i) => `item.${i.n}.photo: ${i.imageUrl}`),
    );
    expect(countMenuContentRows(content)).toBe(plan.items.length);
    // The prompt parser reads back exactly the plan's photos.
    expect(
      parsePosBoundRows(content).map((r) => [r.n, r.photo ?? null]),
    ).toEqual(plan.items.map((i) => [i.n, i.imageUrl ?? null]));
  });
  ...
  it('the Designer is told which rows have a photo — and that the rest get none', async () => {
    ...
    const cdn = await toastPhotoCdn();
    const plan = await loadPosBindingPlan(
      { prisma, menu, photos: { storage: memoryBucket(), fetch: cdn.fetch } },
      't1',
      SELECTION,
      CANVAS,
    );
    const withPhoto = plan.items
      .filter((i) => i.imageUrl)
      .map((i) => `item.${i.n}`);
    expect(withPhoto).toHaveLength(2);
    const p = buildDesignerUserPrompt({
      prompt: 'our Toast menu',
      width: 3840,
      height: 2160,
      purpose: 'menu',
      content: formatPosPlanContent(plan),
    });
    expect(p).toContain(
      `- ITEM PHOTOS — 2 of the rows (${withPhoto.join(' and ')}) end "photo: item.N.photo"`,
    );
    expect(p).toContain('A row without a photo gets no photo frame');
    expect(p).toContain(
      'THIS BOARD IS THE MENU — the content above has 5 items.',
    );
  ...
  it('no storage injected: the same plan, no photos, no fetch — and still marked, so the binder strips borrowed ones', async () => {
    ...
    const cdn = await toastPhotoCdn();
    const plan = await loadPosBindingPlan(
      { prisma, menu, photos: { storage: null, fetch: cdn.fetch } },
      't1',
      SELECTION,
      CANVAS,
    );
    ...
    const bare = await loadPosBindingPlan(
      { prisma, menu },
      't1',
      SELECTION,
      CANVAS,
    ...
    const plan = await loadPosBindingPlan(
      {
        prisma,
        menu,
        photos: { storage: memoryBucket({ fail: true }), fetch: cdn.fetch },
      },
      't1',
      SELECTION,
      CANVAS,
...
describe('a long row keeps its photo marker and still counts as a row', () => {
  it('the description is shortened (never the name, price or marker) to keep the row within 300 characters', () => {
    ...
    plan.items[0].imageUrl =
      'https://sb.example/storage/v1/object/public/assets/ai-designer/t1/item-0123456789abcdef.jpg';
    const content = formatPosPlanContent(plan);
    ...
    expect(row).toContain(
      `— ${long('A Very Long Dish Name', 80)} — $1234.56 — `,
    );
