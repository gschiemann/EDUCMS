...
const pack = (id: string, boards: number, bought: string, months = 12): PackForSettlement => ({
  id,
  boards,
  createdAt: d(bought),
...
describe('the unit', () => {
  ...
  it('a credit is a board DRAWN for the tenant; the loop that makes it good is never one', () => {
    expect([...BOARD_CREDIT_FEATURES].sort()).toEqual(['designer', 'designer-revise']);
    for (const internal of ['designer-redraw', 'designer-review', 'designer-review-revise', 'designer-brief']) {
    ...
    expect(BOARD_PIPELINE_FEATURES).toEqual(expect.arrayContaining(['designer', 'designer-revise', 'designer-redraw', 'designer-review', 'designer-review-revise']));
    ...
    expect(BOARD_COGS_FEATURES).toEqual(expect.arrayContaining([...BOARD_PIPELINE_FEATURES, 'designer-brief']));
  });

  it('the packs: known ids, and a bigger pack is never a worse price per board', () => {
    expect(AI_BOARD_PACKS.map((p) => p.id)).toEqual(['starter', 'standard', 'bulk']);
    expect(new Set(AI_BOARD_PACKS.map((p) => p.id)).size).toBe(AI_BOARD_PACKS.length);
    const perBoard = AI_BOARD_PACKS.map((p) => p.usd / p.boards);
    for (let i = 1; i < perBoard.length; i++) expect(perBoard[i]).toBeLessThan(perBoard[i - 1]);
    ...
    expect(boardPackById('standard')).toEqual({ id: 'standard', boards: 30, usd: 19 });
  ...
  it('months are UTC calendar months', () => {
    ...
    expect(monthStart('2026-12', 1).toISOString()).toBe('2027-01-01T00:00:00.000Z');
    expect(addMonthsUtc(d('2026-09-23T17:05:00.000Z'), 12).toISOString()).toBe('2027-09-23T17:05:00.000Z');
    expect(addMonthsUtc(d('2026-10-15T00:00:00.000Z'), -24).toISOString()).toBe('2024-10-15T00:00:00.000Z');
  });
...
describe('settleBoardPacks — month by month, soonest-expiring pack first', () => {
  ...
  it('no packs → nothing purchased, nothing to settle', () => {
    const s = settleBoardPacks({ packs: [], usedByMonth: map({ '2026-09': 50 }), includedByMonth: map({ '2026-09': 10 }), now });
    expect(s).toEqual({ remaining: new Map(), purchasedRemaining: 0, unfunded: 0 });
  });

  it('a pack is drawn on only AFTER the month\'s included boards are gone', () => {
    const packs = [pack('p1', 30, '2026-09-05T00:00:00Z')];
    expect(settleBoardPacks({ packs, usedByMonth: map({ '2026-09': 10 }), includedByMonth: map({ '2026-09': 10 }), now }).purchasedRemaining).toBe(30);
    expect(settleBoardPacks({ packs, usedByMonth: map({ '2026-09': 14 }), includedByMonth: map({ '2026-09': 10 }), now }).purchasedRemaining).toBe(26);
  });
  ...
  it('two packs: the one that expires first is drawn first', () => {
    const packs = [pack('late', 30, '2026-09-01T00:00:00Z'), pack('early', 10, '2026-03-01T00:00:00Z')];
    const s = settleBoardPacks({ packs, usedByMonth: map({ '2026-09': 25 }), includedByMonth: map({ '2026-09': 10 }), now });
  ...
  it('an EXPIRED pack\'s unused boards lapse — only unexpired packs count as purchased-remaining', () => {
    const packs = [pack('old', 10, '2025-08-01T00:00:00Z'), pack('new', 30, '2026-09-10T00:00:00Z')];
    const s = settleBoardPacks({ packs, usedByMonth: map({}), includedByMonth: map({}), now });
  ...
  it('a pack that expires mid-month still carries the share of that month it covered', () => {
    ...
    const packs = [pack('early', 10, '2025-09-15T00:00:00Z'), pack('late', 10, '2026-09-01T00:00:00Z')];
    const s = settleBoardPacks({ packs, usedByMonth: map({ '2026-09': 14 }), includedByMonth: map({ '2026-09': 10 }), now });
  ...
  it('a pack bought AFTER a month ended never pays for that month (a race past the pre-check is reported, not billed)', () => {
    ...
    const s = settleBoardPacks({ packs, usedByMonth: map({ '2026-08': 13 }), includedByMonth: map({ '2026-08': 10 }), now });
  ...
  it('overage no pack could cover is reported as unfunded', () => {
    ...
    const s = settleBoardPacks({ packs, usedByMonth: map({ '2026-09': 15 }), includedByMonth: map({ '2026-09': 10 }), now });
...
describe('who may buy a pack', () => {
  ...
  it('no platform design key → NO_PLATFORM_KEY (a pack would buy nothing usable), whatever the source says', () => {
    ...
    expect(boardPurchaseAvailability('none')).toMatchObject({ enabled: false, reasonCode: 'NO_PLATFORM_KEY', reason: 'AI runs on your own key — add it in Settings → AI provider.' });
    expect(boardPurchaseAvailability('platform')).toMatchObject({ enabled: false, reasonCode: 'NO_PLATFORM_KEY' });
  });

  it('own key → OWN_KEY; Stripe off → STRIPE_NOT_CONFIGURED; both in place → enabled', () => {
    process.env.OPENAI_API_KEY = 'sk-openai';
    expect(boardPurchaseAvailability('tenant')).toMatchObject({ enabled: false, reasonCode: 'OWN_KEY' });
    expect(boardPurchaseAvailability('platform')).toMatchObject({ enabled: false, reasonCode: 'STRIPE_NOT_CONFIGURED' });
    process.env.STRIPE_SECRET_KEY = 'sk_test_x';
...
describe('the 402 message', () => {
  ...
  it('names the numbers and both ways forward', () => {
    expect(boardsCapMessage({ needed: 2, left: 1, resetAt, purchaseEnabled: true, kind: 'batch' })).toBe(
  ...
  it('one board, none left, and no "buy more" when a pack cannot be bought', () => {
    const m = boardsCapMessage({ needed: 1, left: 0, resetAt, purchaseEnabled: false, kind: 'refine' });
    expect(m).toContain('This edit needs 1 board; you have none left this month');
