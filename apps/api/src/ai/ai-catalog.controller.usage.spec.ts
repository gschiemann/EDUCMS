...
function build(
  events: Ev[],
  packs: Array<{ orgTenantId: string; usdMicros: number; boards: number }>,
) {
  const groupBy = (rows: any[], by: string[], sum: string[]) => {
    ...
    for (const r of rows) {
      ...
      const g = out.get(k) || {
        ...
        _sum: Object.fromEntries(sum.map((s) => [s, 0])),
        _count: { _all: 0 },
      };
    ...
    client: {
      aiUsageEvent: {
        groupBy: jest.fn(async ({ by, where }: any) => {
          let rows = events;
          if (where.source)
            rows = rows.filter((e) => e.source === where.source);
          if (where.feature?.in)
            rows = rows.filter((e) => where.feature.in.includes(e.feature));
          return groupBy(rows, by, ['costMicros']);
        }),
      },
      aiCreditPurchase: {
        groupBy: jest.fn(async ({ by }: any) =>
          groupBy(packs, by, ['usdMicros', 'boards']),
        ),
      },
      tenant: {
        findMany: jest.fn(async ({ where }: any) =>
          where.id.in.map((id: string) => ({ id, name: `Name of ${id}` })),
        ),
      },
    },
  };
  const allowance: any = {
    snapshot: jest.fn(async (org: string) => ({
      screens: org === 'org_A' ? 4 : 0,
      includedMicros: org === 'org_A' ? 8_000_000 : 5_000_000,
    })),
    boardCostTrailing: jest.fn(
      async () =>
        new Map([
...
describe('GET /super/ai/usage — the AI board margin', () => {
  it('per organisation and in total: pack revenue, boards sold/used, COGS, gross margin, trailing cost per board', async () => {
    const e = (
      ...
      source = 'platform',
      provider = 'openai',
    ): Ev => ({ orgTenantId, source, provider, feature, costMicros });
    const controller = build(
      [
        e('org_A', 'designer', 170_000),
        e('org_A', 'designer', 170_000),
        e('org_A', 'designer-revise', 170_000),
        e('org_A', 'designer-review', 30_000),
        e('org_A', 'designer-review-revise', 170_000),
        e('org_A', 'designer-brief', 10_000, 'platform', 'anthropic'),
  ...
  it('stays a SUPER_ADMIN-only controller', () => {
    expect(Reflect.getMetadata(ROLES_KEY, AiCatalogController)).toEqual([
      AppRole.SUPER_ADMIN,
    ]);
  });
});
