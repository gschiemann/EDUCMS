...
export type StaticDefectCode = 'truncated' | 'incomplete-document' | 'no-editable-text' | 'no-image-slot' | 'binding-incomplete';
...
export function documentIncompleteness(html: string): string | null {
  const s = String(html || '');
  if ((/<!doctype\s+html/i.test(s) || /<html[\s>]/i.test(s)) && !/<\/html\s*>/i.test(s)) {
    return 'it opens a document and never closes </html>';
  }
  if (/<body[\s>]/i.test(s) && !/<\/body\s*>/i.test(s)) return 'it opens <body> and never closes it';
  return null;
}
 ...
 */
export function textEditability(html: string): { chars: number; editableChars: number; samples: string[] } {
  ...
  root
    ...
    .each((_i, node: any) => {
      ...
      if (!n || !node.parent) return;
      const $parent = $(node.parent);
      if ($parent.closest('[aria-hidden="true"], [data-imgslot], [data-img]').length) return;
      chars += n;
...
export function designerBoardDefects(html: string, ctx: BoardDefectContext = {}): StaticBoardDefect[] {
  const out: StaticBoardDefect[] = [];
  const doc = String(html || '');

  if (ctx.truncated) {
    const n = typeof ctx.outputTokens === 'number' && ctx.outputTokens > 0 ? ` after ${ctx.outputTokens} output tokens` : '';
    out.push({ code: 'truncated', severity: 'blocker', detail: `the answer was cut off at the output ceiling${n}` });
  }
  const incomplete = documentIncompleteness(doc);
  if (incomplete) out.push({ code: 'incomplete-document', severity: 'blocker', detail: incomplete });

  if (ctx.binding && !ctx.binding.ok) {
    const rows = ctx.binding.missing.slice(0, 12).map((n) => `[item.${n}]`).join(', ');
    out.push({ code: 'binding-incomplete', severity: 'blocker', detail: `planned POS rows missing: ${rows || 'unknown'}` });
  }
  ...
  if (!incomplete) {
    ...
    if (t.chars > 0 && t.editableChars === 0) {
      out.push({ code: 'no-editable-text', severity: 'blocker', detail: 'none of the text carries data-field — nothing on the board can be edited' });
    ...
    if (ctx.logoUrl && !usesUrl(doc, ctx.logoUrl)) {
      out.push({ code: 'no-image-slot', severity: 'major', detail: 'the supplied logo is not on the board (expected <img data-imgslot="logo" src="…"> in the header)' });
    }
    if (ctx.heroImageUrl && !usesUrl(doc, ctx.heroImageUrl)) {
      out.push({ code: 'no-image-slot', severity: 'major', detail: 'the supplied photo is not on the board (expected <img data-imgslot="hero" src="…"> in a framed panel)' });
    }
  }
 ...
 */
export function staticRedrawNudge(defects: readonly StaticBoardDefect[], outputTokens?: number | null): string {
  const lines: string[] = [];
  const cut = defects.some((d) => d.severity === 'blocker' && (d.code === 'truncated' || d.code === 'incomplete-document'));
  if (cut) {
    const n = typeof outputTokens === 'number' && outputTokens > 0 ? ` after ${outputTokens} tokens` : '';
    lines.push(
  ...
  if (defects.some((d) => d.severity === 'blocker' && d.code === 'no-editable-text')) {
    lines.push('your previous answer had text with no data-field — every text element needs data-field="<key>" so the operator can edit it.');
  }
