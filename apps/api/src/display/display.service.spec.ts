...
// ─────────────────────────────────────────────────────────────────────
// RESET_CANVAS (2026-09-01, G55 field find) — the second verb on this
// endpoint that touches no hardware. It makes the web player forget a
// DEVICE-SIDE LED canvas pin (localStorage + URL params the server never
// sees) and reload at the panel's native size. Like OPEN_SETUP: never gated
// on a verdict, never soft/hard, never darkening, forwarded untranslated.
// ─────────────────────────────────────────────────────────────────────
describe('RESET_CANVAS — rides the display transport, drives no display', () => {
  let service: DisplayService;
  let prisma: any;
  let redis: any;
  let signer: any;
  ...
  it('is accepted on a screen that has never reported, and never gated on any verdict', () => {
    expect(displayActionSupport('RESET_CANVAS', null)).toEqual({
      supported: true,
      mechanism: 'web-canvas-reset',
    });
    for (const screenBlank of ['none', 'screen-timeout', 'device-admin', 'unknown']) {
      expect(
        displayActionSupport('RESET_CANVAS', { ...G43_VERDICT, screenBlank } as any).supported,
      ).toBe(true);
    }
  });
  ...
  it('publishes an untranslated frame with no percent, no revert, no soft/hard', async () => {
    const res = await apply();
    expect(res.action).toBe('RESET_CANVAS');
    const frame = published();
    expect(frame.action).toBe('RESET_CANVAS');
    expect(frame.percent ?? null).toBeNull();
    expect(frame.revertAfterMs ?? null).toBeNull();
    expect(frame.soft ?? false).toBe(false);
    expect(frame.hard ?? false).toBe(false);
  });

  it('is never refused by the emergency hold — it does not darken anything', async () => {
    await expect(
      apply({ emergencyHold: { active: true, source: 'tenant' } }),
    ).resolves.toMatchObject({ action: 'RESET_CANVAS' });
  });
});
