...
export function reportsSecondDisplay(inventoryReport: unknown): boolean {
  // ⚠️ THE ARRAY IS NESTED ONE LEVEL DEEPER THAN IT READS.
  //
  // The probe assembles this report in SECTIONS — DisplayCapabilityProbe.kt:201
  // is `section(root, "displays") { displaySurface(ctx) }`, and displaySurface
  // itself ends `out.put("displays", arr)`. So the wire shape is:
  //
  //     report.displays          = { available: true, displays: [ … ] }   OBJECT
  //     report.displays.displays = [ … ]                                  ARRAY
  //
  // The first cut of this function read `report.displays` and required an
  // array, which is false for every device that has ever reported. The whole
  // feature would have shipped dead — "Add the back side" never offered, on
  // hardware that genuinely has a back side — and no test caught it because
  // every fixture was hand-written in the shape the code expected rather than
  // the shape the probe sends. Fixtures below are cut from the real payload.
  //
  // The bare-array form is still accepted: it costs one line and means a
  // caller that has already unwrapped the section is not silently wrong.
  const section = (inventoryReport as { displays?: unknown } | null | undefined)?.displays;
  const displays = Array.isArray(section)
    ? section
    : (section as { displays?: unknown } | null | undefined)?.displays;
