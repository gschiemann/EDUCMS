  ...
  async getManifest(@Param('id') id: string, @Req() req: ExpressReq, @Res() res: Response) {
    ...
    return ({
      ...
      items: s.playlist.items.map(pi => {
        ...
        url: selected.url,
        // 2026-09-26 (Greg) — NO lower-resolution stand-in. bdb3f59a sent a
        // >1080p screen its 1080p rendition as fallback_url; the player showed
        // it until the 4K file cached, then switched up at the loop — viewers
        // read the 1080p as the real quality. A screen plays its native file
        // only; with no fallback field the player always picks it
        // (mediaSourceChoice.ts), and a cached copy keeps serving meanwhile.
