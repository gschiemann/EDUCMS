...
1. **2-decimal per-dive scoring** — the single biggest correctness gap; a CTS/Daktronics board never rounds 60.75 to 60.8.
2. **Team-score → FINAL continuity** — derive the dual-meet team total from per-diver results (or block FINAL until it's set) so the closing cinematic isn't 0.00–0.00.
3. **Final standings scene** — reuse `DiveLeaderboardWidget` at FINAL instead of the two-number team cinematic.
