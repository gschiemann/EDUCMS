...
| P0-4 | 2 | ~~Player bypasses the CDN proxy~~ **DOWNGRADED + SHIPPED (`05946ee`).** Live curl 2026-05-31: raw Supabase objects (oldest 2026-04-17 + newest) already serve `max-age=31536000[,immutable]` with `cf-cache HIT` — the "no-cache" premise is stale, egress root cause is fixed at source. Player now also honors the CDN flag (images-only wiring, inert until `NEXT_PUBLIC_ASSET_CDN` set), but it's belt-and-suspenders, not urgent. Real residual = **large video** (P1-5). | `player/page.tsx` | Wiring shipped; left inert (needless cost). | Done. |
