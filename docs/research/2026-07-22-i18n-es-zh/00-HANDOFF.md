...
- **Playlists — COMPLETE** (list surface `be58685e` + all modals `e9a1d794`), namespace `playlistsPage` (121 keys): list/stats/search/filters/download + slide editor (transitions, per-slide scheduling, Mon–Sun) + schedule/meal-block + Choose-Media + Submit-for-Review + full Publish-to-Screens modal.

All: hook wired into every component, tsc/build/gates green, lockstep + ICU parity, live zh browser proof (desktop + mobile where relevant), zero unresolved keys. **The Chinese customer's whole journey — login → dashboard → pair screens → upload → build playlist → publish to a screen — is now end-to-end zh.** Deferred within these: the dashboard mobile panic-type taxonomy (emergency pass); the 2,783-line PlaylistCreateWizard (separate large file).

## STILL ENGLISH — TODO (not yet done — do not claim otherwise)

**Biggest single remaining surface — templates.** `app/[schoolId]/templates/page.tsx` is **4,947 lines** across 8 components: `TemplatesPage` (gallery browse + AI-intake wizard, 410–2762), `TemplatePreviewModal`, `AdaptForLedModal`, `TemplateBuilder`, `AssetPicker`, `WidgetConfig`, `PlaylistPicker`, `ApplyBrandButton`. Do it in bounded chunks (browse surface first, then AI wizard, then builder/modals) so a wall never leaves a half-done file. Also `PlaylistCreateWizard.tsx` (2,783 lines).

⚠️ **DEPENDENCY found probing templates (do these together):** the templates header subtitle (and many template surfaces) interpolate **`tenantCopy.orgSingular`** — "school"/"gym"/"store"/etc. from `useTenantCopy` (`hooks/use-tenant-copy` + `lib/*copy*`). That copy system is ENGLISH and drives vertical-aware nouns app-wide. Translating a sentence around it in zh yields a Chinese sentence with an English noun inside ("为你的 school 设计…"). So the `useTenantCopy` vertical-noun set must be localized IN THE SAME pass as templates (and it also fixes role labels + "Add a School/Store/Gym" buttons elsewhere). Scope `useTenantCopy` first, then templates reads clean.

**Other pages the dead agents never touched:** templates `imports`, signup, onboarding, reviews, audit, analytics, announcements, floor-plans, sports, menu (+ their components/*).

Same proven pattern every time: `const t = useTranslations()`, new namespace, author en/es/zh in lockstep, **wire the hook into EVERY component in a multi-component file** — after the string pass, `pnpm --filter web exec tsc --noEmit | grep <file>` lists exactly which components still need `const t = useTranslations();`. Verify: tsc + build (0 IntlError) + the lockstep/ICU node one-liner + live zh browser scan for zero unresolved keys. Reusable scripts from this session live in the scratchpad (dash.py/screens.py/assets.py/pl1.py/pl2.py) as templates for the approach.
