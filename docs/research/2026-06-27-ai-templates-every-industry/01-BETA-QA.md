...
## P2 — follow-ups
4. ✅ **Candidate thumbnail misrenders engine boards** — `ScaledTemplateThumbnail` piled all scenes' zones on one canvas + hardcoded white bg (ignored the gradient → invisible light text). Operator couldn't trust the preview. → **FIXED `a9e9d5bf`**: thumbnail honors `candidate.background` (bgColor/bgGradient/bgImage) + renders only the first scene's zones for a set (`sceneRef === first scene`); `sceneRef` added to the FE candidate zone type.
