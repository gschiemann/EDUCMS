...
**Decision (Greg, 2026-09-02): both the foldable LED poster AND the double-sided LCD must play DIFFERENT content on each side.** That fixes the model: **one face = one `Screen` row**, always. Face B is paired from the same device with a derived fingerprint (`<fp>-B`), gets its own name, playlist, schedule, groups, emergency reach and seat — every existing fleet feature applies with no new content model, and the dashboard needs only a "faces of one device" link (`Screen.deviceFaceGroupId` + `faceIndex`) so the two rows sit together and the LED canvas picker can set the split once.
- **Double-sided LCD (true second display):** the APK attaches an Android `Presentation` to display 2 hosting a second WebView on `/player?fp=<fp>-B`. Zero web change; two independent players, one box. Mirrored-by-OS panels (one logical display) cannot do different content — the device report decides which case a unit is.
- **Foldable LED poster (one 320×1080 output):** one WebView must composite TWO manifests: top tile = face A's manifest, bottom tile (rotated 180°) = face B's manifest. That is a dual-manifest player mode: two `manifestGate`-serialized reconcile chains (or one chain fetching both), one device credential covering both rows, both tiles driven by the same `syncedNow`, and the emergency decision evaluated per face but an alert on EITHER face's tenant/group taking BOTH faces (life-safety wins over "different content"). Player rules 4, 7, 11 apply to each face independently.
- Order of work: (1) `deviceFaceGroupId`/`faceIndex` + pairing of `<fp>-B` (API, small); (2) LCD Presentation in the APK; (3) LED dual-manifest compositing in the web player; (4) dashboard "faces" affordance + canvas picker option.
