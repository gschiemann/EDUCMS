/**
 * Idempotent resync of the Cafeteria Chalkboard preset ONLY.
 * Run: pnpm --filter @cms/database exec tsx prisma/resync-cafeteria.ts
 */
...
async function main() {
  const preset = SYSTEM_TEMPLATE_PRESETS.find(p => p.id === 'preset-cafeteria-menu');
  if (!preset) throw new Error('Cafeteria Chalkboard preset not found');
  ...
  console.log(`Resynced "${preset.name}" (${preset.zones.length} zones, theme=diner-chalkboard).`);
}
