/**
 * Idempotent resync of the Final Chance preset ONLY.
 * Run: pnpm --filter @cms/database exec tsx prisma/resync-final-chance.ts
 */
...
async function main() {
  const preset = SYSTEM_TEMPLATE_PRESETS.find(p => p.id === 'preset-final-chance');
  if (!preset) throw new Error('Final Chance preset not found in SYSTEM_TEMPLATE_PRESETS');
  ...
  console.log(`Resynced preset "${preset.name}" (${preset.zones.length} zones, theme=final-chance).`);
}
