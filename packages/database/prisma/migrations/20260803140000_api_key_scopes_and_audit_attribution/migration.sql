...
-- RENAME, no type change, no NOT NULL, no backfill, no data migration. Every
-- existing row stays valid exactly as it is and every existing query keeps
-- working unchanged.
--
-- Ordering note: this file is timestamped AFTER 20260803090000_device_
-- credential_revocation (screens.*) and touches a disjoint set of tables
-- (audit_logs, tenant_api_keys), so the two cannot collide in either order.
--
-- WHY EACH ONE
--
...
--   stamps the key id on the row itself.
--
...
--
--   ADD COLUMN is compatible with those triggers: they fire BEFORE
--   UPDATE/DELETE/TRUNCATE on rows and statements, not on DDL, and Postgres
--   adds a nullable column without rewriting the table.
--
