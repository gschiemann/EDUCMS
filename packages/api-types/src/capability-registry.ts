...
export const CAPABILITY_REGISTRY: ReadonlyArray<Capability> = [
  ...
  {
    id: 'design-file-import',
    name: 'Design file import (PDF / images / Canva-Slides-PowerPoint exports)',
    domain: 'templates',
    state: 'CONFIGURED',
    owner: 'templates',
    surface: 'Templates → Import design (/templates/imports)',
    publicClaim: 'Import a PDF, image, or a Canva / Google Slides / PowerPoint export as a template or playlist.',
    evidenceTest: null,
    verifiedAt: null,
    expiresAt: null,
    limitations:
      'FILE import only — upload the PDF/PNG the design tool exports. Canva Connect OAuth ("Sign in with Canva") and direct Google Slides / PowerPoint cloud sync are NOT built (Sprint 11 roadmap); any copy implying one-click cloud import is an overclaim. Page-preview → create-as-template/playlist flow shipped; no automated CI canary uploads a real file yet — hence CONFIGURED.',
    dependsOn: [],
  },
  {
    id: 'pos-connectors',
    name: 'Point-of-sale connectors (Toast / Square / Clover) + menu price binding',
    domain: 'integrations',
    state: 'CONFIGURED',
    owner: 'integrations',
    ...
    expiresAt: null,
    limitations:
      'Connectors + menu-price binding + per-location overrides are wired and were graded REAL in the 2026-06-26 integration audit; there is no automated CI canary against live provider sandboxes — hence CONFIGURED, not VERIFIED. Registered 2026-07-21 when the menu-boards-pos help guide made the claim scannable (TRUTH-001).',
    dependsOn: [],
  },
  {
