...
**Note:** All queries executed against this schema must be parameterized. Raw SQL concatenation is forbidden. 
...
CREATE TABLE users (
    ...
    password_hash VARCHAR(255) NOT NULL, -- Argon2id hash containing random salts
    session_identifier VARCHAR(255), -- Rotated on successful login
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE schedules (
    ...
    recurrence JSONB -- CRON format or standard RRULE
);
```
...
CREATE TABLE emergency_override_state (
    ...
    payload JSONB NOT NULL, -- The override instruction, visual data, or pre-configured scene
...
-- Note: the actor_id represents the privilege user triggering the action.
```
