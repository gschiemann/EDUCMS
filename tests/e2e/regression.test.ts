...
describe('Automated Regression Suite (REGRESSION_SUITE_PLAN.md)', () => {
  ...
  describe('Audit Log Verification', () => {
    it('Should emit formatted, persistent logs for critical state changes', async () => {
      ...
      await request(app).post('/api/v1/emergency/trigger').set('Authorization', 'Bearer MOCK_ADMIN').send({ scopeType: 'tenant', scopeId: 'xyz', overridePayload: { severity: 'CRITICAL' } });
      await request(app).post('/api/v1/device/revoke').set('Authorization', 'Bearer MOCK_ADMIN').send({ target: 'device-xyz' });
