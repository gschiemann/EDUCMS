 ...
 */
test_1.test.describe('Holistic Workflow: Admin Schedule to Device Offline Sync', () => {
    const API_BASE = 'http://localhost:3000'; // Default test backend
    (0, test_1.test)('Validates complete Go/No-Go pipeline', async ({ request }) => {
        // ----------------------------------------------------
        ...
        if (!loginRes.ok()) {
            test_1.test.skip('Backend service unavailable, skipping E2E workflow assertions');
            return;
        }
        ...
        (0, test_1.expect)(publishRes.status()).toBe(202); // Accepted
        // ----------------------------------------------------
        ...
        (0, test_1.expect)(syncRes.status()).toBe(200);
        const jsonPayload = await syncRes.json();
        // Zod validation mathematically ensures the backend isn't sending corrupted types
        const strictManifest = (0, sync_protocol_1.validateManifest)(jsonPayload);
        (0, test_1.expect)(strictManifest.versionHash).toBeDefined();
        ...
        (0, test_1.expect)(etagSyncRes.status()).toBe(304); // Must not re-download
    });
});
test_1.test.describe('SecOps Enforcements: RBAC Matrix Validations', () => {
    const API_BASE = 'http://localhost:3000';
    (0, test_1.test)('Contributor Role (Teacher) cannot trigger emergency overrides', async ({ request }) => {
        ...
        if (req.status() !== 404) {
            (0, test_1.expect)(req.status()).toBe(403);
        }
    });
    (0, test_1.test)('Emergency / Audit Log tracking works correctly', async ({ request }) => {
