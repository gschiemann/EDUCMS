const fs = require('fs');

let c = fs.readFileSync('apps/api/src/templates/system-presets.ts', 'utf8');

const stemBg = "url('data:image/svg+xml;base64,')";
const libBg = "url('data:image/svg+xml;base64,')";
const musicBg = "url('data:image/svg+xml;base64,')";

if (!c.includes('const STEM_LAB_BG')) {
  // Just insert at the top after imports
  c = c.replace("import { Template } from '@prisma/client';", "import { Template } from '@prisma/client';\n\nconst STEM_LAB_BG = \"" + stemBg + "\";\nconst LIBRARY_QUIET_BG = \"" + libBg + "\";\nconst MUSIC_ARTS_BG = \"" + musicBg + "\";\n");
}

const applyTheme = (id, theme) => {
  const startIndex = c.indexOf(`id: '${id}'`);
  if (startIndex === -1) {
    console.log(`Missing ID: ${id}`);
    return;
  }
  
  // Find the closing bracket of the zones array
  let endIndex = c.indexOf(`\n  },`, startIndex);
  if (endIndex === -1) {
    console.log(`Could not find end of ${id}`);
    return;
  }

  let block = c.substring(startIndex, endIndex);
  
  // Update bgGradient if applicable
  if (theme === 'library-quiet') block = block.replace(/bgColor: '#F5F0EB'/, 'bgGradient: LIBRARY_QUIET_BG');
  if (theme === 'music-arts') block = block.replace(/bgGradient: 'linear-gradient\\(135deg[^\\)]+\\)'/, 'bgGradient: MUSIC_ARTS_BG');
  if (theme === 'stem-science') block = block.replace(/bgGradient: 'linear-gradient\\(160deg[^\\)]+\\)'/, 'bgGradient: STEM_LAB_BG');

  // Inject theme into defaultConfig
  block = block.replace(/defaultConfig:\s*\{\s*(?!theme:)/g, `defaultConfig: { theme: '${theme}', `);
  
  c = c.substring(0, startIndex) + block + c.substring(endIndex);
  console.log(`Patched ${id} with theme: ${theme}`);
};

const mapping = {
  'preset-classroom-daily': 'back-to-school',
  'preset-hallway-trizone': 'final-chance',
  'preset-cafeteria-menu': 'diner-chalkboard',
  'middle-school-hall-board': 'middle-school-hall',
  'high-school-athletics-scoreboard': 'high-school-athletics',
  'bus-loop-dismissal-board': 'bus-loop',
  'preset-lobby-welcome': 'sunshine-academy',
  'preset-lobby-sunny-meadow': 'sunny-meadow',
  'library-quiet-zone': 'library-quiet',
  'music-room-arts': 'music-arts',
  'stem-science-lab': 'stem-science'
};

Object.entries(mapping).forEach(([id, theme]) => applyTheme(id, theme));

fs.writeFileSync('apps/api/src/templates/system-presets.ts', c);
console.log('Done!');
