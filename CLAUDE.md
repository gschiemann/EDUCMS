...
| `PEXELS_API_KEY` | Free [Pexels](https://pexels.com/api) stock-photo key for the **IMAGERY wave** — makes EVERY photo-appropriate AI signage board come back with a real, relevant stock photo for **$0**, for every tenant, regardless of AI provider. The art-director resolves a Pexels photo at generate time (`StockImageService`, `apps/api/src/ai/stock-image.service.ts`) and the engine art-directs it (brand grade + directional contrast scrim, `imageTreatmentCss`); on persist the photo is re-hosted into our Supabase bucket (durable + offline-cacheable on Taurus). The operator can one-tap **"Make it an AI photo"** to upgrade to an on-brand AI photo (that path uses the tenant's own OpenAI/Google BYOK key via `POST /templates/:id/regenerate-image`, never the platform key). Used **server-side only** (key in the `Authorization` header, never the URL, never logged; ~6s AbortSignal). When **unset**, boards ride their themed gradient exactly as before — `search()` returns null, never throws, zero regression. Generous free tier (≈200 req/hr, 20k/mo); get one free + instant at pexels.com/api. | `563492ad…` |
